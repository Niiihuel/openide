...
## Settings y personalizaciones — 22 de septiembre de 2026

La revisión de Skills y Plugins encontró trabajo redundante concreto, además de diferencias visuales con Settings. `CustomizationCardList` recorría todas las filas para reasignar `tabIndex` al cambiar el foco; el evento de foco posterior repetía ese recorrido. Con 300 filas en Chromium, una pulsación de ArrowDown produjo 600 mutaciones de `tabindex` antes del cambio y 2 después. Ahora se actualizan solamente la fila anterior y la siguiente. La prueba de regresión conserva la navegación, un único elemento principal tabulable y el acceso a las acciones de la fila.

El buscador de Skills de Settings también llamaba a `listSkills(true)` para cada consulta asentada; esa ruta lee los archivos SKILL.md del inventario. La sección reutiliza ahora una instantánea mientras cambia únicamente la búsqueda y comparte el descubrimiento que ya está en curso. Cambiar de alcance, reabrir la sección o ejecutar acciones de modificación vuelve a descubrir las skills. Tres pruebas cubren el filtrado, las consultas durante una lectura pendiente y los resultados tardíos después de disponer la sección.

Las personalizaciones usan las superficies, tipografía y espaciado de Settings. Las acciones de desbordamiento quedan transparentes en reposo; mantienen feedback al pasar el puntero y outline de foco por teclado. La prueba `dev/test-agent-settings-navigation-runtime.mjs` valida temas oscuro, claro y high contrast, apertura real del menú y los modales anidados de marketplace e instalación sin mezclar pestañas del proyecto. Capturas y resultados: `.build/agent-settings-navigation-runtime/`.

Validación: 25 pruebas de listas de personalización, 3 de Skills Settings, el flujo Electron y `typecheck-client` aprobados. La reducción medida corresponde a mutaciones DOM al navegar una lista; no es una medición de FPS ni demuestra que toda demora de Settings haya desaparecido.

## Providers — 22 de septiembre de 2026

El alta manual deja de redirigir a Advanced/JSON: un formulario DOM nativo permite crear y editar nombre, endpoint, formato de API, autenticación, clave y modelo. Conserva el borrador ante actualizaciones de detección; las claves viajan separadas a SecretStorage y no entran en `customProviders`. Editar conserva el identificador y las opciones avanzadas existentes. La prueba de conexión no guarda credenciales ni genera inferencias; distingue credenciales ausentes/rechazadas, endpoint sin listado compatible, error HTTP y problemas de red. Un catálogo público de modelos no demuestra saldo ni permiso para generar respuestas.

Guardar una clave desde el detalle o desde el formulario comprueba primero el endpoint. El rechazo mantiene la clave anterior y el borrador visible; ante problemas de red/servidor hay una acción explícita para guardar sin comprobar. Cambiar un campo revoca esa opción y los cambios externos invalidan resultados de conexión anteriores. La UI distingue una clave disponible de una conexión comprobada y muestra el origen cuando procede del entorno o de otro CLI. Elegir un modelo aplica también su provider; se eliminó el borrador que dependía de un botón de aplicación ya inexistente.

Se retiró `getConnectedModelGroups()` al cargar un detalle: consultar un provider ya no descubre todos los demás. Las solicitudes simultáneas de modelos comparten trabajo (10 llamadas en la prueba producen 1 solicitud), tienen tiempo límite y no repueblan cachés con respuestas anteriores a un cambio de clave/endpoint. Actualizar detección invalida también lecturas de credenciales externas pendientes. No hay polling nuevo. Estos son contadores y regresiones de comportamiento, no mediciones de FPS o de latencia global.

Providers conserva su montaje en Settings durante actualizaciones ajenas: evita perder foco, selección o borradores antes de que la sección pueda procesar el evento. Es un contrato opt-in para secciones que observan sus propios datos; cambios de categoría, búsqueda, ámbito, idioma o perfil siguen remontando. Las páginas con filas nativas siguen actualizándolas. Los selectores del formulario usan el mismo dropdown y popover del resto de Settings.

La suite de sección cubre navegación entre cargas pendientes, selección efectiva de provider/modelo, rechazo sin escritura, resultados obsoletos y conservación de búsqueda/foco/caret. La suite del editor cubre retención y ciclo de vida. El E2E `dev/test-providers-settings-runtime.mjs` usa perfil efímero, servidor HTTP local y claves ficticias; el transporte de la prueba bloquea endpoints externos. Las capturas y resultados se escriben en `.build/providers-settings-runtime/`. El chequeo de tipos del cliente pasa; las pruebas de servicio/auth, formulario, sección y editor se ejecutan en Chromium.
