...
[[ -f "${MANIFEST}" && -f "${SIGNATURE}" && -f "${PUBLIC_KEY}" ]] || { echo "Missing manifest, signature or public key." >&2; exit 1; }
...
const sig=JSON.parse(fs.readFileSync(sigPath,'utf8')); if(sig.algorithm!=='ed25519'||!sig.keyId||!sig.signature) throw new Error('Invalid signature file');
if(!crypto.verify(null,fs.readFileSync(manifest),fs.readFileSync(keyPath),Buffer.from(sig.signature,'base64'))) throw new Error('Signature does not verify');
const m=JSON.parse(fs.readFileSync(manifest,'utf8')); if(m.schemaVersion!==2||m.product!=='openide') throw new Error('Manifest is not OpenIDE v2');
