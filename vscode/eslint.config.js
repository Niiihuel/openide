...
// OpenIDE ships without the Copilot extension. Its local ESLint plugin only matters for the
// `extensions/copilot/**` blocks below, which never match a file here, so the import is optional:
// a missing plugin degrades to an empty rule set instead of taking the whole lint run down.
const pluginCopilotLocal = existsSync(fileURLToPath(new URL('./extensions/copilot/.eslintplugin/index.ts', import.meta.url)))
...
export default defineConfig(
	...
	// OpenIDE's own contributions carry their own copyright line.
	{
		files: [
			'src/vs/workbench/contrib/openide*/**/*.{ts,js}',
			'src/vs/**/openide*.{ts,js}',
		],
		rules: {
			'header/header': [
				2,
