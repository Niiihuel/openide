...
async function runTestsInBrowser(testModules, browserType, browserChannel) {
	...
	/** Modules that could not be imported: they register no suites, so mocha cannot fail on them. */
	let loadFailures = [];
	try {
		// @ts-expect-error
		loadFailures = await page.evaluate(opts => loadAndRun(opts), {
			modules: testModules,
			grep: args.grep,
		}) ?? [];
	} catch (err) {
	...
	// A module that never loaded is reported ALONGSIDE the failing tests, never instead of them:
	// returning early here hid the assertions from the same run that surfaced the load errors.
	const loadFailureReport = loadFailures.length > 0
		? `${loadFailures.length} test module(s) failed to load and ran NOTHING:\n - ${loadFailures.map(({ file, message }) => `${file} (${message})`).join('\n - ')}`
		: '';
	...
	if (failingTests.length > 0) {
		...
		return `${loadFailureReport ? `${loadFailureReport}\n\n` : ''}${res}\n`;
	}

	return loadFailureReport || undefined;
}
