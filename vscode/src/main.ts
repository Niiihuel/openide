...
// The OS shows `app.name` on notifications, and in dev Electron takes it from package.json
// (`code-oss-dev`). The product name is what the user recognises, so it wins in every build.
// After the userData path: `setName` would otherwise move the default data directory.
app.setName(product.nameShort || 'OpenIDE');
...
function createDefaultArgvConfigSync(argvConfigPath: string): void {
	try {
		...
		const defaultArgvConfigContent = [
			'// This configuration file allows you to pass permanent command line arguments to OpenIDE.',
			'// Only a subset of arguments is currently supported to reduce the likelihood of breaking',
			'// the installation.',
			'//',
			'// PLEASE DO NOT CHANGE WITHOUT UNDERSTANDING THE IMPACT',
			'//',
			'// NOTE: Changing this file requires a restart of OpenIDE.',
			'{',
			'	// Use software rendering instead of hardware accelerated rendering.',
			'	// This can help in cases where you see rendering issues in OpenIDE.',
