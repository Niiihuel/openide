...
import { CodebaseMemoryServerChannel } from './contrib/openideCodebaseMemoryServerChannel.js';
...
class SharedProcessMain extends Disposable implements IClientConnectionFilter {
	...
	private initChannels(accessor: ServicesAccessor): void {
		...
		// OpenIDE codebase memory. The channel creates one isolated runtime per workspace
		// initialized by a renderer window; indexing never runs in the renderer.
		const codebaseMemoryChannel = this._register(new CodebaseMemoryServerChannel(accessor.get(IFileService)));
		this.server.registerChannel('openideCodebaseMemory', codebaseMemoryChannel);
