...
// Keep the Electron preload a script: inline type imports emit no ESM export.
type BrowserElementSelectionMode = import('../common/browserView.js').BrowserElementSelectionMode;
type IBrowserElementCommentsUpdate = import('../common/browserView.js').IBrowserElementCommentsUpdate;
type IBrowserElementSelectionOptions = import('../common/browserView.js').IBrowserElementSelectionOptions;
type IBrowserViewPreloadLocalizedStrings = import('../common/browserView.js').IBrowserViewPreloadLocalizedStrings;
type IBrowserViewTheme = import('../common/browserView.js').IBrowserViewTheme;
type IBrowserViewRect = import('../common/browserView.js').IBrowserViewRect;
...
function init() {
	...
	const getElementData = (id: string) => {
		...
		const allComputedStyles: Record<string, string> = {};
		for (let index = 0; index < computed.length; index++) {
			const property = computed.item(index);
			allComputedStyles[property] = computed.getPropertyValue(property).trim();
		}
		...
		for (let current: Element | null = element; current; current = current.parentElement) {
			ancestors.unshift({
				...
				classNames: current.classList.length ? [...current.classList] : undefined,
			});
		}
		...
		return {
			...
			computedStyle: Object.entries(allComputedStyles).map(([property, value]) => `${property}: ${value};`).join('\n'),
			bounds: { x: rect.left, y: rect.top, width: rect.width, height: rect.height },
			dimensions: { top: rect.top, left: rect.left, width: rect.width, height: rect.height },
			ancestors,
			attributes,
			computedStyles: allComputedStyles,
			allComputedStyles,
		};
	};
	...
	const isolatedHelpers = {
		...
		getElementData,
		setElementStyle(id: string, property: string, value: string) {
			const element = getElement(id);
