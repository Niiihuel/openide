...
// Keep this file a script (not an ES module). Electron loads registered frame preloads as
// CommonJS in development; even a type-only import makes tsc emit `export {}` and the preload
// then fails before `init()` with `SyntaxError: Unexpected token 'export'`.
interface IBrowserViewTheme {
...
function init() {
	...
	const elementPicker = new ElementPicker(
		(el, selectionRect) => ipcRenderer.send('vscode:browserView:elementPicked', track(el), selectionRect),
...
class ElementPicker {
	...
	constructor(
		private readonly _onPicked: (element: Element, selectionRect?: { x: number; y: number; width: number; height: number }) => void,
	...
	private _commit(target: Element, selectionRect?: { x: number; y: number; width: number; height: number }): void {
		if (!this._selectionActive) {
			return;
		}
		...
		requestAnimationFrame(() => {
			...
			this._onPicked(target, selectionRect);
		});
	}
