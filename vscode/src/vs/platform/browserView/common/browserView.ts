...
export enum BrowserViewCommandId {
	...
	TakeScreenshot = `${commandPrefix}.takeScreenshot`,
	CaptureAreaScreenshot = `${commandPrefix}.captureAreaScreenshot`,
	...
	ClearHistory = `${commandPrefix}.clearHistory`,
...
export interface IElementData {
	/** Stable page-preload identity for live inspection operations within the current document. */
	...
	/** Complete computed style map used by the visual inspector's editable CSS view. */
	readonly allComputedStyles?: Record<string, string>;
	readonly dimensions?: { readonly top: number; readonly left: number; readonly width: number; readonly height: number };
	readonly innerText?: string;
	/** Exact page-coordinate rectangle dragged by the user, when selection used a region. */
	readonly selectionRect?: { readonly x: number; readonly y: number; readonly width: number; readonly height: number };
}
...
export interface IBrowserViewConsoleMessageEvent {
...
export interface IBrowserViewJavaScriptResult {
...
export interface IBrowserViewBounds {
	...
	zoomFactor: number;
	cornerRadius: number;
	/** Bounds reserved by the workbench for Chromium DevTools, when docked in the editor. */
	devTools?: {
		x: number;
		y: number;
		width: number;
...
export interface IBrowserViewService {
	...
	onDynamicDidAddConsoleMessage(id: string): Event<IBrowserViewConsoleMessageEvent>;
	...
	/** Clear the navigation history of a browser view. */
	clearHistory(id: string): Promise<void>;

	/** Clear cookies in the browser view's shared storage session. */
	clearCookies(id: string): Promise<void>;

	/** Clear the HTTP cache in the browser view's shared storage session. */
	...
	/** Execute JavaScript in the current page, as entered in the integrated console. */
	evaluateJavaScript(id: string, expression: string): Promise<IBrowserViewJavaScriptResult>;
	...
	/** Apply or remove an inline CSS declaration on an inspected element and return fresh data. */
	setElementStyle(id: string, elementId: string, property: string, value: string): Promise<IElementData>;
