...
export class BrowserViewMainService extends Disposable implements IBrowserViewMainService {
	...
	onDynamicDidAddConsoleMessage(id: string) {
		return this._getBrowserView(id).onDidAddConsoleMessage;
	}
	...
	async clearHistory(id: string): Promise<void> {
		this._getBrowserView(id).webContents.navigationHistory.clear();
	}

	async clearCookies(id: string): Promise<void> {
		return this._getBrowserView(id).session.clearCookies();
	}

	async clearCache(id: string): Promise<void> {
		return this._getBrowserView(id).session.clearCache();
	}
	...
	async evaluateJavaScript(id: string, expression: string) {
		return this._getBrowserView(id).evaluateJavaScript(expression);
	}
	...
	async setElementStyle(id: string, elementId: string, property: string, value: string): Promise<IElementData> {
		return this._getBrowserView(id).inspector.setElementStyle(elementId, property, value);
	}
