...
export class BrowserViewInspector extends Disposable {
	...
	private readonly _selectedElements = new Map<string, BrowserViewFrameInspector>();
	...
	private _onInspectorAdopted(inspector: BrowserViewFrameInspector): void {
		inspector.onDidInspectElement(async nodeData => {
			if (nodeData.elementId) { this._selectedElements.set(nodeData.elementId, inspector); }
	...
	async setElementStyle(elementId: string, property: string, value: string): Promise<IElementData> {
		...
		const inspector = this._selectedElements.get(elementId);
		if (!inspector || inspector.frame.isDestroyed()) {
			throw new Error('The inspected element is no longer available.');
		}
		const data = await inspector.setElementStyle(elementId, property, value);
		const offset = await this._getFrameOffsetInPage(inspector.frame);
		return this._offsetElementData({ ...data, elementId }, offset);
	}
