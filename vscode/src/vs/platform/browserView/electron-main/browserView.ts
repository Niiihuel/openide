...
export class BrowserView extends Disposable {
	...
	private _devToolsView: WebContentsView | undefined;
	...
	private readonly _onDidAddConsoleMessage = this._register(new Emitter<IBrowserViewConsoleMessageEvent>());
	readonly onDidAddConsoleMessage: Event<IBrowserViewConsoleMessageEvent> = this._onDidAddConsoleMessage.event;
	...
	private setupEventListeners(): void {
		...
		this._view.webContents.on('console-message', (event) => {
			const entry: IBrowserViewConsoleMessageEvent = {
				level: event.level,
				message: event.message,
				lineNumber: event.lineNumber,
				sourceId: event.sourceId,
			};
			this._consoleLogs.push(`[${entry.level}] ${entry.message}`);
			...
			this._onDidAddConsoleMessage.fire(entry);
		});
	}
	...
	getState(): IBrowserViewState {
		...
		return {
			...
			isDevToolsOpen: this._isDevToolsOpen,
	...
	toggleDevTools(): void {
		const webContents = this._view.webContents;
		if (this._isDevToolsOpen || this._devToolsView) {
			this._isDevToolsOpen = false;
			...
			this.debugger.releaseForDevTools();
			const devToolsView = new WebContentsView();
			devToolsView.setBackgroundColor('#1E1E1E');
			devToolsView.setVisible(false);
			this._devToolsView = devToolsView;
			this._isDevToolsOpen = true;

			const window = this._currentWindow ?? this._ownerWindow;
			window.win?.contentView.addChildView(devToolsView);
			webContents.setDevToolsWebContents(devToolsView.webContents);
			// `detach` is required when supplying our own WebContents. The view itself remains
			// docked because its bounds are owned by BrowserEditor and share the editor split.
	...
	layout(bounds: IBrowserViewBounds): void {
		...
		if (this._currentWindow?.win?.id !== bounds.windowId) {
			...
			if (newWindow) {
				...
				if (this._devToolsView) {
					this._currentWindow?.win?.contentView.removeChildView(this._devToolsView);
				}
				this._currentWindow = newWindow;
				newWindow.win?.contentView.addChildView(this._view);
				if (this._devToolsView) {
					newWindow.win?.contentView.addChildView(this._devToolsView);
				}
			}
		}
	...
	private _layoutDevToolsView(): void {
		const devToolsView = this._devToolsView;
		const bounds = this._lastBounds;
		const devToolsBounds = bounds?.devTools;
		if (!devToolsView || !bounds || !devToolsBounds || !this._isDevToolsOpen) {
			devToolsView?.setVisible(false);
			return;
		}

		devToolsView.setBounds({
			x: Math.round(devToolsBounds.x * bounds.zoomFactor),
			y: Math.round(devToolsBounds.y * bounds.zoomFactor),
			width: Math.round(devToolsBounds.width * bounds.zoomFactor),
			height: Math.round(devToolsBounds.height * bounds.zoomFactor),
		});
		devToolsView.setVisible(this._view.getVisible());
	}

	private _disposeDevToolsView(): void {
		const devToolsView = this._devToolsView;
		if (!devToolsView) {
			return;
		}
		this._devToolsView = undefined;
		const currentWin = this._currentWindow?.win ?? this._ownerWindow.win;
		if (currentWin && !currentWin.isDestroyed()) {
			currentWin.contentView.removeChildView(devToolsView);
		}
		if (!devToolsView.webContents.isDestroyed()) {
			devToolsView.webContents.close({ waitForBeforeUnload: false });
		}
	}
	...
	async evaluateJavaScript(expression: string): Promise<IBrowserViewJavaScriptResult> {
		try {
			const result = await this._view.webContents.executeJavaScript(expression, true);
			if (result === undefined) {
	...
	override dispose(): void {
		...
		if (!this._view.webContents.isDestroyed() && this._view.webContents.isDevToolsOpened()) {
