...
export class BrowserViewFrameInspector extends Disposable {
	...
	async setElementStyle(elementId: string, property: string, value: string): Promise<IElementData> {
		...
		if (!result?.objectId) { throw new Error('The inspected element is no longer available.'); }
		try {
			await this.connection.sendCommand('Runtime.callFunctionOn', {
				objectId: result.objectId,
				functionDeclaration: 'function(property, value) { if (value) { this.style.setProperty(property, value); } else { this.style.removeProperty(property); } }',
	...
	try {
		...
		if (computedStyleArray) {
			...
			for (const prop of computedStyleArray) {
				...
				allComputedStyles[prop.name] = prop.value;
