...
suite('OpenIDE update manifest', () => {
	const context: IOpenideUpdateManifestContext = { channel: 'stable', platform: 'linux', architecture: 'x64', target: 'appimage', currentVersion: '1.121.1', minimumUpdaterVersion: 1 };
	const valid = () => ({
	...
	test('rejects downgrade and cross target', () => {
		assert.throws(() => parseOpenideUpdateManifest({ ...valid(), productVersion: '1.121.1' }, context), /anti-rollback/);
	...
	test('rejects type coercion and channel mismatch', () => {
		...
		assert.throws(() => parseOpenideUpdateManifest({ ...valid(), codeOssVersion: '1.122.0' }, context), /línea API/);
		assert.throws(() => parseOpenideUpdateManifest({ ...valid(), buildVersion: 'c'.repeat(40) }, { ...context, highestSeenVersion: '1.121.2', highestSeenBuildVersion: 'a'.repeat(40), highestSeenArtifactSha256: 'b'.repeat(64) }), /identidad/);
		const insider = { ...context, channel: 'insider' as const, currentVersion: '1.121.1-insider.20260721.1' };
		assert.throws(() => parseOpenideUpdateManifest({ ...valid(), channel: 'insider', productVersion: '1.121.2' }, insider), /política/);
	});
});
