...
export function parseOpenideUpdateManifest(value: unknown, context: IOpenideUpdateManifestContext): IOpenideUpdateManifestV2 {
	...
	// `codeOssVersion` is informational: it records which VS Code API level the published build
	// implements. It is deliberately NOT tied to `productVersion` -- OpenIDE versions itself on its
	// own line, so requiring the two to share a major.minor would reject every valid manifest.
	const codeOssVersion = string(root.codeOssVersion, 'codeOssVersion');
	if (!SEMVER.test(codeOssVersion)) { throw new OpenideUpdateManifestError('invalid-version', 'codeOssVersion no es SemVer válido.'); }
