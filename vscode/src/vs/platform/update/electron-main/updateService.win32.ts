					...
					return this.getUpdatePackagePath(update.version, updateType).then(updatePackagePath => {
						return pfs.Promises.exists(updatePackagePath).then(exists => {
							...
							return this.requestService.request({ url: update.url, callSite: NO_FETCH_TELEMETRY }, CancellationToken.None)
								...
								.then(() => {
									if (this.productService.applicationName === 'openide' && (!update.sha256hash || !update.size)) { throw new Error('El manifest firmado no incluye tamaño/SHA-256 obligatorio.'); }
									return update.sha256hash ? checksum(downloadPath, update.sha256hash) : undefined;
								})
