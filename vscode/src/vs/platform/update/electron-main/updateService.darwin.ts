...
export class DarwinUpdateService extends AbstractUpdateService implements IRelaunchHandler {
	...
	protected buildUpdateFeedUrl(quality: string, _commit: string, _options?: IUpdateURLOptions): string | undefined {
		return createUpdateURL(this.productService, quality, process.platform, process.arch, 'archive');
	}
	...
	private async checkForUpdateNoDownload(url: string, canInstall?: boolean): Promise<void> {
		if (this.productService.applicationName === 'openide') {
			try { const update = await this.readSignedOpenideManifest(url); this.setState(update ? State.AvailableForDownload(update, canInstall) : State.Idle(UpdateType.Archive, undefined, true)); }
			catch (error) { this.logService.error(error); this.setState(State.Idle(UpdateType.Archive, String(error))); }
			return;
		}
