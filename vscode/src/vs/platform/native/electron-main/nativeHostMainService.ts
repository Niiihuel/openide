...
export class NativeHostMainService extends Disposable implements INativeHostMainService {
	...
	/**
	 * The product icon for OS notifications. Without it Linux shows the generic app glyph — the
	 * desktop entry only resolves the icon for installed builds, never for a dev launch.
	 */
	private toastIcon(): string | undefined {
		for (const candidate of [
			join(app.getAppPath(), 'resources', 'linux', 'code.png'),
			join(app.getAppPath(), '..', '..', 'resources', 'linux', 'code.png'),
		]) {
