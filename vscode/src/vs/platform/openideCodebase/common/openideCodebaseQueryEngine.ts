...
import { ICodebaseMemoryEdge, ICodebaseMemoryNode, CodebaseMemoryProvider, CodebaseMemoryRelationType, ICodebaseMemoryQueryResult } from './openideCodebaseMemoryTypes.js';
import { ICodebaseMemorySnapshotDto, ICodebaseQueryRequest } from './openideCodebaseMemoryProtocol.js';

export interface ICodebaseQuerySource {
	readonly onDidChange: Event<unknown>;
	getSnapshot(): Promise<ICodebaseMemorySnapshotDto | undefined>;
}
export type CodebaseQueryConfiguration = Pick<IConfigurationService, 'getValue' | 'onDidChangeConfiguration'>;
...
export interface ICodebaseQueries {
...
export class CodebaseQueryEngine extends Disposable implements ICodebaseQueries {
	...
	constructor(
		private readonly memory: ICodebaseQuerySource,
		private readonly configurationService: CodebaseQueryConfiguration,
	) {
		super();
...
/** Shared-process fallback dispatch. Only validated, bounded query requests reach the engine. */
export function validateCodebaseQueryRequest(value: unknown): asserts value is ICodebaseQueryRequest {
	if (!value || typeof value !== 'object') { throw new Error('Invalid codebase query.'); }
	const request = value as ICodebaseQueryRequest;
	if (!['search', 'explore', 'callers', 'callees', 'impact', 'path', 'relatedTests', 'communityLabel', 'pickSeeds'].includes(request.method) || !Array.isArray(request.arguments) || request.arguments.length > 8 || typeof request.includeHeuristic !== 'boolean' || !Number.isFinite(request.maxTraversalDepth) || request.maxTraversalDepth < 1 || request.maxTraversalDepth > 6) { throw new Error('Invalid codebase query.'); }
	if (request.arguments.some(value => typeof value === 'string' && value.length > 65536 || Array.isArray(value) && (value.length > 256 || value.some(item => typeof item !== 'string' || item.length > 65536))) || JSON.stringify(request.arguments).length > 262144) { throw new Error('Codebase query exceeds argument bounds.'); }
	const [a, b, c, d, e] = request.arguments;
	const optional = (value: unknown, type: string) => value === undefined || value === null || typeof value === type;
	const optionalNumber = (value: unknown) => value === undefined || value === null || typeof value === 'number' && Number.isFinite(value);
	const optionalStrings = (value: unknown) => value === undefined || value === null || Array.isArray(value) && value.every(item => typeof item === 'string');
	let valid = ['impact', 'relatedTests'].includes(request.method) ? Array.isArray(a) && a.every(value => typeof value === 'string') : typeof a === 'string';
	switch (request.method) {
		case 'search': valid &&= b === undefined || b === null || typeof b === 'object' && !Array.isArray(b) && optionalStrings((b as ICodebaseSearchOptions).kinds) && optionalStrings((b as ICodebaseSearchOptions).languages) && optional((b as ICodebaseSearchOptions).pathPrefix, 'string') && optionalNumber((b as ICodebaseSearchOptions).limit); break;
		case 'explore': valid &&= (b === undefined || b === null || ['both', 'incoming', 'outgoing'].includes(String(b))) && optionalStrings(c) && optionalNumber(d) && optionalNumber(e); break;
		case 'callers': case 'callees': valid &&= optional(b, 'boolean') && optionalNumber(c) && optionalNumber(d); break;
		case 'impact': valid &&= optional(b, 'boolean') && optional(c, 'boolean') && optionalNumber(d); break;
		case 'path': valid &&= typeof b === 'string' && optionalStrings(c) && optionalNumber(d); break;
		case 'relatedTests': case 'pickSeeds': valid &&= optionalNumber(b); break;
	}
	if (!valid) { throw new Error('Invalid codebase query arguments.'); }
}

export function executeCodebaseQuery(engine: ICodebaseQueries, request: ICodebaseQueryRequest): Promise<unknown> {
	validateCodebaseQueryRequest(request);
	// JSON transports encode omitted positional arguments as null. Restore JS defaults.
	const [a, b, c, d, e] = request.arguments.map(value => value === null ? undefined : value);
	switch (request.method) {
		case 'search': return engine.search(a as string, b as ICodebaseSearchOptions | undefined);
		case 'explore': return engine.explore(a as string, b as 'incoming' | 'outgoing' | 'both' | undefined, c as CodebaseMemoryRelationType[] | undefined, d as number | undefined, e as number | undefined);
		case 'callers': return engine.callers(a as string, b as boolean | undefined, c as number | undefined, d as number | undefined);
		case 'callees': return engine.callees(a as string, b as boolean | undefined, c as number | undefined, d as number | undefined);
		case 'impact': return engine.impact(a as string[], b as boolean | undefined, c as boolean | undefined, d as number | undefined);
		case 'path': return engine.path(a as string, b as string, c as CodebaseMemoryRelationType[] | undefined, d as number | undefined);
		case 'relatedTests': return engine.relatedTests(a as string[], b as number | undefined);
		case 'communityLabel': return engine.communityLabel(a as string);
		case 'pickSeeds': return engine.pickSeeds(a as string, b as number | undefined);
	}
}

export async function executeCodebaseSnapshotQuery(snapshot: ICodebaseMemorySnapshotDto | undefined, request: ICodebaseQueryRequest): Promise<unknown> {
	const engine = new CodebaseQueryEngine({ onDidChange: Event.None, getSnapshot: async () => snapshot }, {
		onDidChangeConfiguration: Event.None,
		getValue: <T>(key?: unknown) => (key === 'openide.memory.showHeuristicRelations' ? request.includeHeuristic : request.maxTraversalDepth) as T,
	});
	try { return await executeCodebaseQuery(engine, request); } finally { engine.dispose(); }
}
