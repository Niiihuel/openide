...
/** Pane header height (px) of the classic workbench, before any Modern UI override. */
export const DEFAULT_PANE_HEADER_SIZE = 22;
let globalPaneHeaderSize = DEFAULT_PANE_HEADER_SIZE;

/**
 * Overrides the header height used by every pane. Modern UI runs taller headers.
 *
 * The value is a BASE size: the fork scales pane headers with the user's sidebar font
 * size (`FONT.sidebarSize22`), and dropping that would silently break UI scaling for
 * anyone who set a custom size. `paneHeaderSize()` re-applies the same sidebar ratio to
 * whatever base is current, so both systems keep working: 22 and 28 are the defaults,
 * and both follow `workbench.sideBar.fontSize`.
 */
export function setGlobalPaneHeaderSize(size: number): void {
	globalPaneHeaderSize = size;
}

/** The current pane header height, scaled by the fork's sidebar font ratio. */
function paneHeaderSize(): number {
	return globalPaneHeaderSize * (FONT.sidebarSize22 / DEFAULT_PANE_HEADER_SIZE);
}
...
export abstract class Pane extends Disposable implements IView {
	...
	private get headerSize(): number {
		return this.headerVisible ? paneHeaderSize() : 0;
	}
