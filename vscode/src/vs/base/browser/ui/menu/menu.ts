...
.monaco-menu .monaco-action-bar.vertical .action-menu-item {
	...
	align-items: center;
	position: relative;
	/* The 4px gutter is the container's padding, as in .openide-menu-content; keeping a margin
	   here too would double it and pull the hover surface off the popover's edge. */
	margin: 0;
...
/* The .openide-menu-row inset: a 21px leading column for the check (.openide-menu-row-icon) and
   8px of trailing air, instead of 2em — 26px — on both sides. The label's left padding is what
   reserves room for the check, which is absolutely positioned, so the two must stay equal. */
.monaco-menu .monaco-action-bar.vertical .action-label:not(.separator) {
	font-size: inherit;
