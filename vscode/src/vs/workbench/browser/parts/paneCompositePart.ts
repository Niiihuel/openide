...
export abstract class AbstractPaneCompositePart extends CompositePart<PaneComposite> implements IPaneCompositePart {
	...
	protected updateCompositeBar(updateCompositeBarOption: boolean = false): void {
		...
		let isCompositeBarVisible = this.shouldShowCompositeBar();
		const previousPosition = this.compositeBarPosition;
		let newPosition = isCompositeBarVisible ? this.getCompositeBarPosition() : undefined;

		// OpenIDE: the aux bar runs with hasTitle:false (the chat brings its own header) --
		// without a titleContainer there is nowhere to mount the composite bar in TITLE position.
		if (newPosition === CompositeBarPosition.TITLE && !this.titleContainer) {
			isCompositeBarVisible = false;
