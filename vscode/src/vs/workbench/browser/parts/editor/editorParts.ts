...
export class EditorParts extends MultiWindowParts<EditorPart, IEditorPartsMemento> implements IEditorGroupsService, IEditorPartsView {
	...
	private readonly modalPartInstantiationServices = new Map<IEditorPart, IInstantiationService>();
	...
	getScopedInstantiationService(part: IEditorPart): IInstantiationService {
		...
		const modalInstantiationService = this.modalPartInstantiationServices.get(part);
		if (modalInstantiationService) {
			return modalInstantiationService;
		}
	...
	async createModalEditorPart(options?: IModalEditorPartOptions): Promise<IModalEditorPart> {
		...
		this.modalEditorParts.push(part);
		this.modalPartInstantiationServices.set(part, instantiationService);
		...
		disposables.add(toDisposable(() => {
			...
			this.modalPartInstantiationServices.delete(part);
			const index = this.modalEditorParts.indexOf(part);
			if (index >= 0) {
				this.modalEditorParts.splice(index, 1);
			}
		}));
	...
	private saveModalState(): void {
		...
		if (activeModalEditorPart) {
			...
			this.modalEditorPosition = activeModalEditorPart.position;
			if (activeModalEditorPart.hasSidebar) {
				this.modalEditorSidebarWidth = activeModalEditorPart.sidebarWidth;
				this.modalEditorSidebarHidden = activeModalEditorPart.sidebarHidden || undefined;
			}
		}
