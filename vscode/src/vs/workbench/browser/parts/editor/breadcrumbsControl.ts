...
import { OpenidePlanBreadcrumbActions } from '../../../contrib/openideAgent/browser/openidePlanBreadcrumbActions.js';
...
export class BreadcrumbsControl {
	...
	private readonly _hoverDelegate: IHoverDelegate;
	/** The plan's model chip and run button, owned by the agent contrib; this row only hosts them. */
	private readonly _planActions: OpenidePlanBreadcrumbActions;
	...
	constructor(
		...
		this._planActions = this._disposables.add(this._instantiationService.createInstance(OpenidePlanBreadcrumbActions, this.domNode.ownerDocument));
	...
	private _updatePlanActions(): void {
		this._planActions.update(EditorResourceAccessor.getCanonicalUri(this._editorGroup.activeEditor, { supportSideBySide: SideBySideEditor.PRIMARY }));
	}
	...
	private async _revealInEditor(event: IBreadcrumbsItemEvent, element: FileElement | OutlineElement2, group: SIDE_GROUP_TYPE | ACTIVE_GROUP_TYPE | undefined, pinned: boolean = false): Promise<void> {
		...
		if (element instanceof FileElement) {
			if (element.kind === FileKind.FILE) {
				const isPlan = /\.openide[\\/]plans[\\/][^\\/]+\.md$/i.test(element.uri.path);
				await this._editorService.openEditor({ resource: element.uri, options: { pinned, ...(isPlan ? { override: 'workbench.editor.openidePlan' } : {}) } }, group);
			} else {
				// show next picker
