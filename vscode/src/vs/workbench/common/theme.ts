...
// < --- OpenIDE shell --- >
//
// OpenIDE renders the editor, side bars and panel as independent "islands". Using the
// upstream sideBar/panel colors for those surfaces makes the shell change contrast from
// part to part in themes that intentionally tint those areas (Dracula is a clear example).
// These product colors keep the shell coherent while still resolving through the active
// theme. Theme authors can opt into a different OpenIDE surface explicitly if desired.
export const OPENIDE_ISLAND_BACKGROUND = registerColor('openide.islandBackground', editorBackground, localize('openideIslandBackground', "Background color of the editor and dock islands in the OpenIDE shell."));

export const OPENIDE_WORKBENCH_BACKGROUND = registerColor('openide.workbenchBackground', ACTIVITY_BAR_BACKGROUND, localize('openideWorkbenchBackground', "Background color visible in the gaps between OpenIDE shell islands."));

export const OPENIDE_SHELL_SEPARATOR = registerColor('openide.shellSeparator', {
	dark: transparent(editorForeground, 0.16),
	light: transparent(editorForeground, 0.18),
	hcDark: contrastBorder,
	hcLight: contrastBorder
}, localize('openideShellSeparator', "Subtle separator color used between OpenIDE shell regions."));
