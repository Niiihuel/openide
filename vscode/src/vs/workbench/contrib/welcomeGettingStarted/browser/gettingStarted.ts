...
export class GettingStartedPage extends EditorPane {
	...
	private async buildCategoriesSlide(preserveFocus?: boolean) {
		if (this.workspaceContextService.getWorkbenchState() === WorkbenchState.EMPTY) {
			await this.buildOpenIDEEmptyWorkbench();
			return;
		}
		this.container.classList.remove('openide-empty-window');
	...
	private async buildOpenIDEEmptyWorkbench(): Promise<void> {
		...
		const shell = $('.openide-empty-shell');
		const brand = append(shell, $('.openide-empty-brand'));
		append(brand, $('.openide-empty-logo', { 'aria-hidden': 'true' }));
		const brandText = append(brand, $('.openide-empty-brand-text'));
		append(brandText, $('h1', {}, this.productService.nameLong));
		append(brandText, $('p', {}, localize('openide.empty.tagline', "Tu entorno de desarrollo")));

		const actions = append(shell, $('.openide-empty-actions'));
		const actionEntries = [
			{ icon: Codicon.folderOpened, label: localize('openide.empty.openProject', "Abrir proyecto"), command: 'workbench.action.files.openFolder' },
			{ icon: Codicon.repoClone, label: localize('openide.empty.cloneRepo', "Clonar repositorio"), command: 'git.clone' },
			{ icon: Codicon.remote, label: localize('openide.empty.connectSsh', "Conectar por SSH"), command: 'workbench.action.sessions.connectViaSSH' },
		];
		for (const entry of actionEntries) {
			const button = append(actions, $('button.openide-empty-action'));
			button.append(...renderLabelWithIcons(`$(${entry.icon.id})`));
			append(button, $('span', {}, entry.label));
			this.categoriesSlideDisposables.add(addDisposableListener(button, 'click', () => this.commandService.executeCommand(entry.command)));
		}

		const recentSection = append(shell, $('.openide-empty-recent'));
		append(recentSection, $('h2', {}, localize('openide.empty.recent', "Proyectos recientes")));
		const recentList = append(recentSection, $('.openide-empty-recent-list'));
		const { workspaces } = await this.recentlyOpened;
		const recents = this.filterRecentlyOpened(workspaces).slice(0, 7);
		if (!recents.length) {
			append(recentList, $('p.openide-empty-no-recent', {}, localize('openide.empty.noRecent', "Todavía no hay proyectos recientes.")));
		}
		for (const recent of recents) {
			...
			const row = append(recentList, $('button.openide-empty-recent-row', { title: fullPath }));
			append(row, $('span.openide-empty-recent-name', {}, name));
			append(row, $('span.openide-empty-recent-path', {}, parentPath));
			this.categoriesSlideDisposables.add(addDisposableListener(row, 'click', (event: MouseEvent) => {
		...
		reset(this.categoriesSlide, shell);
		this.categoriesPageScrollbar?.scanDomNode();
		this.setSlide('categories');
	}
	...
	private async buildAnnouncementList(): Promise<GettingStartedIndexList<AnnouncementEntry>> {
		...
		const announcementList = this.announcementList = new GettingStartedIndexList({
			title: localize('announcements', "OpenIDE Announcements"),
			klass: 'announcements',
			limit: 5,
		...
		if (!this.announcementData) {
			...
			if (showExtras) {
				...
				await fetch(`https://raw.githubusercontent.com/Niiihuel/openide/${branch}/announcements-extra.json`)
					.then(async res => {
						if (res.ok) {
