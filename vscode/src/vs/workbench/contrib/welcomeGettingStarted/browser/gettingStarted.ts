...
export class GettingStartedPage extends EditorPane {
	...
	private async buildOpenIDEEmptyWorkbench(): Promise<void> {
		...
		append(brandText, $('p', {}, t('welcome.empty.tagline')));

		const actions = append(shell, $('.openide-empty-actions'));
		const actionEntries = [
			{ icon: Codicon.folderOpened, label: t('welcome.empty.openProject'), command: 'workbench.action.files.openFolder' },
			{ icon: Codicon.repoClone, label: t('welcome.empty.cloneRepo'), command: 'git.clone' },
			{ icon: Codicon.remote, label: t('welcome.empty.connectSsh'), command: 'workbench.action.sessions.connectViaSSH' },
		];
		...
		append(recentSection, $('h2', {}, t('welcome.empty.recent')));
		...
		if (!recents.length) {
			append(recentList, $('p.openide-empty-no-recent', {}, t('welcome.empty.noRecent')));
		}
