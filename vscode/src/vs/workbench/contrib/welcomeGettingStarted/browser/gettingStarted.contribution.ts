...
import { showOpenIDEWelcomeOverlay } from './openideWelcomeOverlay.js';
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: { value: t('welcome.cmd.signInGitHub'), original: 'OpenIDE: Connect to GitHub' },
	...
	async run(accessor: ServicesAccessor) {
		...
		try {
			...
			notificationService.info(t('welcome.gh.success', session.account.label));
		} catch (e) {
			notificationService.error(t('welcome.gh.error', e instanceof Error ? e.message : String(e)));
		}
	}
});

// OpenIDE: imports settings/keybindings + extensions from an editor (a VS Code fork).
async function importFromEditor(accessor: ServicesAccessor, editor: IOpenIDEEditor): Promise<void> {
	...
	await progressService.withProgress(
		{ location: ProgressLocation.Notification, title: t('welcome.import.progress', editor.name) },
		async (progress) => {
			const imported: string[] = [];
			const failed: string[] = [];

			// 1) settings.json and keybindings.json
			const files = [
			...
			// 2) Extensions from <extDir>/extensions/extensions.json
			try {
				...
				if (infos.length) {
					progress.report({ message: t('welcome.import.extensions', infos.length) });
				...
				if (toFileOperationResult(e as Error) !== FileOperationResult.FILE_NOT_FOUND) {
					notificationService.warn(t('welcome.import.noExtensions', editor.name));
				}
			}

			if (imported.length) {
				const tail = failed.length
					? ' ' + t('welcome.import.someFailed', failed.length)
					: '';
				notificationService.info(t('welcome.import.done', imported.length, editor.name) + tail);
			} else if (failed.length) {
				notificationService.warn(t('welcome.import.failed', editor.name));
			} else {
				notificationService.info(t('welcome.import.nothing', editor.name));
			}
		}
	);
}
...
registerAction2(class extends Action2 {
	constructor() {
		super({
			...
			title: { value: t('welcome.cmd.importFromEditor'), original: 'OpenIDE: Import settings and extensions from another editor' },
...
// OpenIDE: command to open the full-screen welcome whenever you want.
registerAction2(class extends Action2 {
	constructor() {
		super({
			id: 'openide.showWelcome',
			title: { value: t('welcome.cmd.showWelcome'), original: 'OpenIDE: Show welcome' },
			category: Categories.Help,
			f1: true,
		});
	}
	...
	run(accessor: ServicesAccessor) {
		showOpenIDEWelcomeOverlay({
			...
			updateService: accessor.get(IUpdateService),
			agentService: accessor.get(IOpenideAgentService),
		});
	}
});
