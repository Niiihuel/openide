...
export class ProductContribution implements IWorkbenchContribution {
	...
	constructor(
		...
		hostService.hadLastFocus().then(async hadLastFocus => {
			...
			const currentVersion = tryParseVersion(getOpenideVersion(productService));
			...
			if (shouldShowReleaseNotes && !shouldShowPostInstallInfo && !environmentService.skipReleaseNotes && releaseNotesUrl && lastVersion && currentVersion && isMajorMinorUpdate(lastVersion, currentVersion)) {
				showReleaseNotesInEditor(instantiationService, getOpenideVersion(productService), false)
					.then(undefined, () => {
						notificationService.prompt(
							severity.Info,
							nls.localize('read the release notes', "Welcome to {0} v{1}! Would you like to read the Release Notes?", productService.nameLong, getOpenideVersion(productService)),
							[{
			...
			storageService.store(ProductContribution.KEY, getOpenideVersion(productService), StorageScope.APPLICATION, StorageTarget.MACHINE);
		});
	}
}
	...
	private async onUpdateStateChange(state: UpdateState): Promise<void> {
		...
		switch (state.type) {
			case StateType.Ready: {
				...
				if (productVersion) {
					const currentVersion = tryParseVersion(getOpenideVersion(this.productService));
			...
			this._register(registerAction2(class SwitchQuality extends Action2 {
					...
					try {
						...
						const res = await dialogService.confirm({
							...
							detail: newQuality === 'insider' ?
								nls.localize('relaunchDetailInsiders', "Press the reload button to switch to the Insiders version of OpenIDE.") :
								nls.localize('relaunchDetailStable', "Press the reload button to switch to the Stable version of OpenIDE."),
				...
				private async selectSettingsSyncService(dialogService: IDialogService): Promise<UserDataSyncStoreType | undefined> {
					const { result } = await dialogService.prompt<UserDataSyncStoreType>({
						...
						detail: nls.localize('selectSyncService.detail', "The Insiders version of OpenIDE will synchronize your settings, keybindings, extensions, snippets and UI State using separate insiders settings sync service by default."),
						buttons: [
							{
