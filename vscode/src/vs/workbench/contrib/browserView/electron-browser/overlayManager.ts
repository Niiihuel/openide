...
export class BrowserOverlayManager extends Disposable implements IBrowserOverlayManager {
	...
	getOverlappingOverlays(element: HTMLElement): IBrowserOverlayInfo[] {
		...
		for (const overlay of this.overlays()) {
			...
			if (overlapCenter) {
				// Dialogs and modal editors are blocking by definition. Native
				// WebContentsViews always render above the workbench DOM, so relying on
				// a sampled hit-test here can leave the browser visible during the
				// modal's opening/layout transition.
				if (overlay.type === BrowserOverlayType.Dialog) {
