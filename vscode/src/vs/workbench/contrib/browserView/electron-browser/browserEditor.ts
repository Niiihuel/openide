...
export abstract class BrowserEditorContribution extends Disposable {
	...
	/**
	 * Optional panels displayed alongside the native browser surface. Contributions own the
	 * panel contents and visibility; the editor owns the split layout and WebContentsView bounds.
	 */
	get sidePanelElements(): readonly HTMLElement[] { return []; }

	/**
	 * Optional placeholder whose bounds host Chromium's native DevTools WebContentsView.
	 * The placeholder participates in the workbench split layout but never renders its own UI.
	 */
	get devToolsElement(): HTMLElement | undefined { return undefined; }
...
export class BrowserEditor extends EditorPane {
	...
	protected override createEditor(parent: HTMLElement): void {
		...
		// Keep the native browser view and workbench-owned side panels in one split layout.
		...
		content.appendChild(this._browserContainerWrapper);

		for (const contribution of this._contributionInstances.values()) {
			for (const element of contribution.sidePanelElements) {
		...
		if (this._model) {
			...
			if (show) {
				...
				if (this._overlayVisible) {
					// A native WebContentsView cannot be layered below workbench DOM.
					// Hide it synchronously when a blocking overlay appears so modal
					// editors and dialogs can never be covered by browser contents.
				...
				this.window.requestAnimationFrame(() => model.setVisible(false));
			}
		}
	}
	...
	async clearHistory(): Promise<void> {
		return this._model?.clearHistory();
	}

	async clearCookies(): Promise<void> {
		return this._model?.clearCookies();
	}

	async clearCache(): Promise<void> {
		return this._model?.clearCache();
	}
	...
	private createWelcomeContainer(): HTMLElement {
		...
		title.textContent = localize('browser.welcomeTitle', "Nothing connected");
	...
	layoutBrowserContainer(retries = 2): void {
		if (this._model) {
			...
			const devToolsElement = [...this._contributionInstances.values()]
				.map(contribution => contribution.devToolsElement)
				.find((element): element is HTMLElement => !!element && !element.classList.contains('hidden'));
			const devToolsRect = devToolsElement?.getBoundingClientRect();
			...
			void this._model.layout({
				...
				devTools: devToolsRect && devToolsRect.width > 0 && devToolsRect.height > 0 ? {
					x: devToolsRect.left,
					y: devToolsRect.top,
