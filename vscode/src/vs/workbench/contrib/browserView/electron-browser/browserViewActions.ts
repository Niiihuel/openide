...
class CopyCurrentUrlAction extends Action2 {
	static readonly ID = BrowserViewCommandId.CopyCurrentUrl;

	constructor() {
		super({
			id: CopyCurrentUrlAction.ID,
			title: localize2('browser.copyCurrentUrlAction', 'Copy Current URL'),
			category: BrowserActionCategory,
			icon: Codicon.copy,
			f1: true,
			precondition: ContextKeyExpr.and(BROWSER_EDITOR_ACTIVE, CONTEXT_BROWSER_HAS_URL),
			menu: { id: MenuId.BrowserActionsToolbar, group: BrowserActionGroup.Page, order: 11 }
		});
	}
...
registerAction2(CopyCurrentUrlAction);
