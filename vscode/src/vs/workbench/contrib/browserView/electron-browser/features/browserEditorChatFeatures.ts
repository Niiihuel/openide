...
import { BrowserElementSelectionPurpose, IBrowserViewModel, BrowserViewSharingState } from '../../../browserView/common/browserView.js';
...
export class BrowserEditorChatIntegration extends BrowserEditorContribution {
	...
	protected override subscribeToModel(model: IBrowserViewModel, store: DisposableStore): void {
		...
		store.add(model.onDidSelectElement(async data => {
			if (model.elementSelectionPurpose !== BrowserElementSelectionPurpose.Chat) {
				return;
			}
			try {
		...
		const updateElementSelectionContext = (active = model.isElementSelectionActive) => {
			this._elementSelectionActiveContext.set(active && model.elementSelectionPurpose === BrowserElementSelectionPurpose.Chat);
		};
		updateElementSelectionContext();
		store.add(model.onDidChangeElementSelectionActive(active => {
			updateElementSelectionContext(active);
		}));
	}
...
class AddElementToChatAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, browserEditor = accessor.get(IEditorService).activeEditorPane): Promise<void> {
		if (browserEditor instanceof BrowserEditor) {
			...
			void browserEditor.model?.toggleElementSelection(undefined, BrowserElementSelectionPurpose.Chat);
		}
	}
}
