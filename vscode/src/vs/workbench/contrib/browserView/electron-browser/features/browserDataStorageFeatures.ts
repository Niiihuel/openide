...
class ClearBrowserHistoryAction extends Action2 {
	constructor() {
		super({
			id: BrowserViewCommandId.ClearHistory,
			title: localize2('browser.clearHistoryAction', 'Clear Browsing History'),
			category: BrowserActionCategory,
			icon: Codicon.history,
			f1: true,
			menu: { id: MenuId.BrowserActionsToolbar, group: BrowserActionGroup.Settings, order: 10 }
		});
	}

	async run(accessor: ServicesAccessor, browserEditor = accessor.get(IEditorService).activeEditorPane): Promise<void> {
		if (browserEditor instanceof BrowserEditor) {
			await browserEditor.model?.clearHistory();
		}
	}
}

class ClearBrowserCookiesAction extends Action2 {
	constructor() {
		super({
			id: BrowserViewCommandId.ClearCookies,
			title: localize2('browser.clearCookiesAction', 'Clear Cookies'),
			category: BrowserActionCategory,
			icon: Codicon.circleSlash,
			f1: true,
			menu: { id: MenuId.BrowserActionsToolbar, group: BrowserActionGroup.Settings, order: 11 }
		});
	}

	async run(accessor: ServicesAccessor, browserEditor = accessor.get(IEditorService).activeEditorPane): Promise<void> {
		if (browserEditor instanceof BrowserEditor) {
			await browserEditor.model?.clearCookies();
			await browserEditor.model?.reload();
		}
	}
}

class ClearBrowserCacheAction extends Action2 {
	constructor() {
		super({
			id: BrowserViewCommandId.ClearCache,
			title: localize2('browser.clearCacheAction', 'Clear Cache'),
			category: BrowserActionCategory,
			icon: Codicon.trash,
			f1: true,
			menu: { id: MenuId.BrowserActionsToolbar, group: BrowserActionGroup.Settings, order: 12 }
		});
	}

	async run(accessor: ServicesAccessor, browserEditor = accessor.get(IEditorService).activeEditorPane): Promise<void> {
		if (browserEditor instanceof BrowserEditor) {
			await browserEditor.model?.clearCache();
