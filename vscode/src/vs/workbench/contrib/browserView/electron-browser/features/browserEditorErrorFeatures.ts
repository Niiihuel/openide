...
class BrowserEditorErrorFeatures extends BrowserEditorContribution {
	...
	private _renderLoadError(error: IBrowserViewLoadError): HTMLElement {
		...
		this._errorDisposables.add(addDisposableListener(retry, EventType.CLICK, () => {
			void this.editor.model?.loadURL(error.url);
		}));
	...
	private _waitForServer(url: string): void {
		...
		const probe = async () => {
			...
			try {
				...
				if (this._waitingForUrl === url) {
					this._stopWaitingForServer();
					void this.editor.model?.loadURL(url);
				}
				return;
			} catch {
				// still nothing there
	...
	private _renderError(error: IBrowserViewLoadError): HTMLElement {
		if (error.errorCode === CONNECTION_REFUSED && localPortOf(error.url) && !error.certificateError) {
			return this._renderLoadError(error);
		}
		this._stopWaitingForServer();
