...
export class BrowserViewWorkbenchService extends Disposable implements IBrowserViewWorkbenchService {
	...
	async openPreview(url?: string, initialState?: IBrowserEditorViewState): Promise<BrowserEditorInput> {
		const input = this.getOrCreateLazy(this._previewId, { ...initialState, url });
		if (url) {
			input.navigate(url);
		}
