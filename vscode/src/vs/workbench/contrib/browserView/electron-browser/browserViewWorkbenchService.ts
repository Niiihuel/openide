...
import { IBrowserViewWorkbenchService, IBrowserEditorViewState, IBrowserViewModel, BrowserViewModel, IBrowserViewContextualFilter, IBrowserViewFilterContext, IBrowserViewOpenHandler, IBrowserViewWorkbenchCreateOptions } from '../common/browserView.js';
...
export class BrowserViewWorkbenchService extends Disposable implements IBrowserViewWorkbenchService {
	...
	private get _previewId(): string {
		return `openide-workspace-preview-${this.workspaceContextService.getWorkspace().id}`;
	}
	...
	private _getOrCreateLazy(data: IBrowserEditorInputData, model?: IBrowserViewModel, createOptions?: IBrowserViewWorkbenchCreateOptions): BrowserEditorInput {
		...
		if (!this._known.has(id)) {
			...
			this._register(Event.once(input.onWillDispose)(() => {
