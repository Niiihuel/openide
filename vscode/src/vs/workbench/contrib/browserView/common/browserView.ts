...
import {
	...
	browserZoomFactors,
	IBrowserViewState,
	IBrowserViewConsoleMessageEvent,
	IBrowserViewJavaScriptResult,
...
/** Identifies the workbench feature that owns the current element-picking session. */
export const enum BrowserElementSelectionPurpose {
	Chat = 'chat',
	Inspector = 'inspector',
...
export interface IBrowserViewModel extends IDisposable {
	...
	readonly elementSelectionPurpose: BrowserElementSelectionPurpose;
	...
	readonly onDidAddConsoleMessage: Event<IBrowserViewConsoleMessageEvent>;
	...
	shareWithAgentSession(sessionId: string): Promise<void>;
	...
	evaluateJavaScript(expression: string): Promise<IBrowserViewJavaScriptResult>;
	toggleElementSelection(enabled?: boolean, options?: IBrowserElementSelectionOptions | BrowserElementSelectionPurpose): Promise<void>;
	setElementStyle(elementId: string, property: string, value: string): Promise<IElementData>;
...
export class BrowserViewModel extends Disposable implements IBrowserViewModel {
	...
	private _elementSelectionPurpose = BrowserElementSelectionPurpose.Chat;
	...
	constructor(
		...
		this._register(this.onDidChangeElementSelectionState(state => {
			if (state.active && !this._elementSelectionState.active && this._elementSelectionPurpose === BrowserElementSelectionPurpose.Chat) {
	...
	async clearHistory(): Promise<void> {
		return this.browserViewService.clearHistory(this.id);
	}

	async clearCookies(): Promise<void> {
		return this.browserViewService.clearCookies(this.id);
	}

	async clearCache(): Promise<void> {
		return this.browserViewService.clearCache(this.id);
	}
	...
	async evaluateJavaScript(expression: string): Promise<IBrowserViewJavaScriptResult> {
		return this.browserViewService.evaluateJavaScript(this.id, expression);
	}

	get isElementSelectionActive(): boolean {
		return this._elementSelectionState.active;
	}

	get elementSelectionPurpose(): BrowserElementSelectionPurpose {
		return this._elementSelectionPurpose;
	}

	get onDidChangeElementSelectionActive(): Event<boolean> {
		return Event.map(this.onDidChangeElementSelectionState, state => state.active);
	}

	async toggleElementSelection(enabled?: boolean, options?: IBrowserElementSelectionOptions | BrowserElementSelectionPurpose): Promise<void> {
		const purpose = typeof options === 'string' ? options : BrowserElementSelectionPurpose.Chat;
		const selectionOptions = typeof options === 'string' ? undefined : options;
		const targetEnabled = enabled ?? (!this.isElementSelectionActive || this._elementSelectionPurpose !== purpose);
		if (targetEnabled && this.isElementSelectionActive && this._elementSelectionPurpose !== purpose) {
			await this.browserViewService.toggleElementSelection(this.id, false);
		}
		this._elementSelectionPurpose = purpose;
		return this.browserViewService.toggleElementSelection(this.id, targetEnabled, selectionOptions);
	}

	async setElementStyle(elementId: string, property: string, value: string): Promise<IElementData> {
		return this.browserViewService.setElementStyle(this.id, elementId, property, value);
	}
	...
	get onDidAddConsoleMessage(): Event<IBrowserViewConsoleMessageEvent> {
		return this.browserViewService.onDynamicDidAddConsoleMessage(this.id);
	}

	private static readonly SHARE_DONT_ASK_KEY = 'browserView.shareWithAgent.dontAskAgain';

	async shareWithAgentSession(sessionId: string): Promise<void> {
		await this.browserViewService.setAudience(this.id, { type: 'agent', sessionId }, true);
	}
