...
Registry.as<IEditorPaneRegistry>(EditorExtensions.EditorPane).registerEditorPane(
	EditorPaneDescriptor.create(
		OpenideSettingsEditor,
		OpenideSettingsEditor.ID,
...
class PreferencesActionsContribution extends Disposable implements IWorkbenchContribution {
	...
	private registerSettingsActions() {
		...
		this._register(registerAction2(class extends Action2 {
			...
			run(accessor: ServicesAccessor) {
				...
				if (editorPane instanceof OpenideSettingsEditor) {
	...
	private registerSettingsEditorActions() {
		function getPreferencesEditor(accessor: ServicesAccessor): OpenideSettingsEditor | null {
			const activeEditorPane = accessor.get(IEditorService).activeEditorPane;
			if (activeEditorPane instanceof OpenideSettingsEditor) {
