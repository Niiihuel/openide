...
import { ILocalUrlMatch, UrlFinder } from '../../browser/urlFinder.js';
...
suite('UrlFinder', () => {
	...
	test('should debounce terminal data processing', async () => {
		...
		const detailedMatches: ILocalUrlMatch[] = [];
		store.add(urlFinder.onDidMatchLocalUrl((url: { host: string; port: number }) => matchedUrls.push(url)));
		store.add(urlFinder.onDidMatchLocalUrlDetailed(match => detailedMatches.push(match)));
		...
		assert.strictEqual(detailedMatches.length, 2);
		assert.strictEqual(detailedMatches[0].source.type, 'terminal');
		assert.strictEqual(detailedMatches[0].source.type === 'terminal' && detailedMatches[0].source.instance, mockInstance);
		assert.ok(detailedMatches[0].output.includes('http://localhost:3000/'));
	});
