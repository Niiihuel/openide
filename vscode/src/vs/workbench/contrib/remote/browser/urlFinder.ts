...
export type LocalUrlMatchSource =
	| { readonly type: 'terminal'; readonly instance: ITerminalInstance }
	| { readonly type: 'debug'; readonly session: IDebugSession };

export interface ILocalUrlMatch {
	readonly host: string;
	readonly port: number;
	readonly output: string;
	readonly source: LocalUrlMatchSource;
}
...
export class UrlFinder extends Disposable {
	...
	private readonly _onDidMatchLocalUrlDetailed = this._register(new Emitter<ILocalUrlMatch>());
	readonly onDidMatchLocalUrlDetailed = this._onDidMatchLocalUrlDetailed.event;
	...
	private getOrCreateWorker(instance: ITerminalInstance): RunOnceWorker<string> {
		...
		if (!worker) {
			worker = new RunOnceWorker<string>(chunks => this.processTerminalData(instance, chunks), UrlFinder.dataDebounceTimeout);
			this.terminalDataWorkers.set(instance, worker);
		}
		return worker;
	}

	private processTerminalData(instance: ITerminalInstance, chunks: string[]): void {
		...
		this.processData(chunks.join(''), { type: 'terminal', instance });
	}
	...
	private processNewReplElements(session: IDebugSession) {
		...
		if (!oldReplPosition && replElements.length > 0) {
			replElements.forEach(element => this.processData(element.toString(), { type: 'debug', session }));
			...
			for (let i = replElements.length - 1; i >= 0; i--) {
				...
				if (element === oldReplPosition.tail) {
					...
					this.processData(element.toString(), { type: 'debug', session });
				}
			}
		}
	}
	...
	private processData(data: string, source: LocalUrlMatchSource) {
		...
		if (urlMatches && urlMatches.length > 0) {
			urlMatches.forEach((match) => {
				...
				if (serverUrl) {
					...
					if (!isNaN(port) && Number.isInteger(port) && port > 0 && port <= 65535) {
						...
						this.fireMatch({ port, host, output: data, source });
					}
				}
			});
		} else {
			...
			if (pythonMatch && pythonMatch.length === 3) {
				this.fireMatch({ host: pythonMatch[1], port: Number(pythonMatch[2]), output: data, source });
			}
		}
	}

	private fireMatch(match: ILocalUrlMatch): void {
		this._onDidMatchLocalUrl.fire({ host: match.host, port: match.port });
		this._onDidMatchLocalUrlDetailed.fire(match);
	}
}
