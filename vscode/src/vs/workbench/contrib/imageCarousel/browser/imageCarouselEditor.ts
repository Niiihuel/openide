	...
	private buildSlideshow(): void {
		...
		const elements = h('div.slideshow-container', [
			...
			h('div.bottom-bar@bottomBar', [
				h('div.image-info-bar@infoBar', [
					h('span.caption-text@captionText'),
					h('span.caption-separator@captionSeparator'),
					h('span.image-counter@counter'),
					h('span.caption-separator.zoom-separator@zoomSeparator'),
					h('button.image-zoom@zoomLevel'),
				]),
		...
		elements.zoomSeparator.setAttribute('aria-hidden', 'true');
...
window.addEventListener("message",function(e){var m=e.data;if(m.type==="loadVideo"){var b=new Blob([m.data],{type:m.mimeType});document.getElementById("v").src=URL.createObjectURL(b);}});
	...
	private _applyZoom(newScale: ZoomScale): void {
		...
		if (newScale === 'fit') {
			...
			container.classList.remove('scroll-x');
			container.classList.remove('scroll-y');
			if (wasZoomed) {
	...
	/**
	 * Refresh the zoom percentage in the info bar. Hidden in fit mode, where the percentage
	 * describes the editor size rather than the image.
	 */
	private _updateZoomIndicator(): void {
		if (!this._elements) {
			return;
		}
		const zoomed = this._zoomScale !== 'fit';
		this._elements.infoBar.classList.toggle('has-zoom', zoomed);
		if (!zoomed) {
			this._elements.zoomLevel.textContent = '';
			return;
		}
		const percent = Math.round((this._zoomScale as number) * 100);
		this._elements.zoomLevel.textContent = localize('imageCarousel.zoomPercent', "{0}%", percent);
		this._elements.zoomLevel.title = localize('imageCarousel.fitToWindow', "Fit to window");
		this._elements.zoomLevel.setAttribute('aria-label', localize('imageCarousel.zoomAria', "Zoom {0}%. Fit to window", percent));
	}
