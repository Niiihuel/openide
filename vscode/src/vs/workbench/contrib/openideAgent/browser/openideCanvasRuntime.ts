...
const hostApi = (globalThis as unknown as { acquireVsCodeApi(): { postMessage(message: unknown): void } }).acquireVsCodeApi();
const documentId = (globalThis as unknown as { __openideCanvasDocument?: string }).__openideCanvasDocument;
const vscode = { postMessage: (message: object) => hostApi.postMessage({ ...message, documentId }) };
...
/* SVG needs createElementNS: with createElement the <svg> ends up in the XHTML namespace, is
   not an SVGElement and IS NOT DRAWN — that is why canvas charts were never visible. The svg
   mode propagates to the children, because the namespace is set by the ancestor, not the tag. */
const SVG_NS='http://www.w3.org/2000/svg';
/* SVG attributes that ARE camelCase; the rest are converted to kebab-case (strokeWidth →
   stroke-width), which is how the parser expects them. */
...
function setProp(el: Element, key: string, value: unknown, isSvg: boolean): void {
		...
		if(key==='children'||key==='key'||value==null)return;if(key==='style'&&typeof value==='object'){Object.assign(node.style,value as object);return}if(key.startsWith('on')&&typeof value==='function'){const name=key.slice(2).toLowerCase();el.addEventListener(name,value as EventListener);let entries=listeners.get(el);if(!entries){entries=new Map();listeners.set(el,entries)}entries.set(name,value as EventListener);return}if(isSvg){el.setAttribute(key==='className'?'class':svgAttr(key),String(value));return}if(key==='className'){node.className=value;return}if(key==='checked'||key==='disabled'||key==='selected'){node[key]=!!value;return}if(key in node&&key!=='list'&&key!=='form'){try{node[key]=value;return}catch(e){}}el.setAttribute(key,String(value))}
function dom(v: CanvasVNode, svgMode?: boolean): Node {if(typeof v==='string'||typeof v==='number')return document.createTextNode(String(v));if(!v||v===true)return document.createComment('');if(Array.isArray(v)){const frag=document.createDocumentFragment();v.forEach(function(child: CanvasVNode){frag.appendChild(dom(child,svgMode))});return frag}if(v.type===Fragment){const f=document.createDocumentFragment();(v.props.children||[]).forEach(function(c: CanvasVNode){f.appendChild(dom(c,svgMode))});return f}if(typeof v.type==='function'){const prev=currentComponent;currentComponent=v.type;hookIndex=0;let out;try{out=v.type(v.props||{})}finally{currentComponent=prev}return dom(out,svgMode)}const isSvg=svgMode||v.type==='svg';const tag = (v as ICanvasElement).type as string;
		...
		Object.keys(nodeProps).forEach(function(k: string){setProp(el,k,nodeProps[k],isSvg)});(nodeProps.children||[]).forEach(function(c: CanvasVNode){el.appendChild(dom(c,isSvg&&tag!=='foreignObject'))});if(el instanceof HTMLSelectElement && nodeProps.value !== undefined)el.value=String(nodeProps.value);if(nodeProps.key!=null)el.setAttribute('data-oc-key',String(nodeProps.key));return el as Node}
// Preserve DOM identity during interaction (focus, caret and pointer capture).
function reconcile(parent: Node, desired: Node): void {
	const incoming = Array.from(desired.childNodes);
	const old = Array.from(parent.childNodes);
	const identity = (n: Node) => n instanceof Element ? n.getAttribute('data-oc-key') || n.id : '';
	let cursor = parent.firstChild;
	for (const fresh of incoming) {
		const key = identity(fresh);
		const candidate = key ? old.find(n => identity(n) === key && n.parentNode === parent) : cursor;
		if (!candidate || candidate.nodeName !== fresh.nodeName || (candidate instanceof Element && fresh instanceof Element && candidate.namespaceURI !== fresh.namespaceURI) || (!key && candidate && identity(candidate))) {
			parent.insertBefore(fresh, cursor); continue;
		}
		if (candidate !== cursor) { parent.insertBefore(candidate, cursor); }
		if (candidate instanceof Element && fresh instanceof Element) {
			for (const [name, handler] of listeners.get(candidate) ?? []) { candidate.removeEventListener(name, handler); }
			for (const [name, handler] of listeners.get(fresh) ?? []) { candidate.addEventListener(name, handler); }
			listeners.set(candidate, listeners.get(fresh) ?? new Map());
			for (const attr of Array.from(candidate.attributes)) { if (!fresh.hasAttribute(attr.name)) { candidate.removeAttribute(attr.name); } }
			for (const attr of Array.from(fresh.attributes)) { if (candidate.getAttribute(attr.name) !== attr.value) { candidate.setAttribute(attr.name, attr.value); } }
			if (candidate instanceof HTMLInputElement && fresh instanceof HTMLInputElement) { if (candidate.value !== fresh.value) { candidate.value = fresh.value; } candidate.checked = fresh.checked; }
			if (candidate instanceof HTMLTextAreaElement && fresh instanceof HTMLTextAreaElement && candidate.value !== fresh.value) { candidate.value = fresh.value; }
			const selectedValue = fresh instanceof HTMLSelectElement ? fresh.value : undefined;
			reconcile(candidate, fresh);
			if (candidate instanceof HTMLSelectElement && selectedValue !== undefined) { candidate.value = selectedValue; }
		} else if (candidate.nodeValue !== fresh.nodeValue) { candidate.nodeValue = fresh.nodeValue; }
		cursor = candidate.nextSibling;
	}
	while (cursor) { const next = cursor.nextSibling; parent.removeChild(cursor); cursor = next; }
}
let Top: CanvasComponent | null = null;
function render(){if(!Top)return;const next=document.createDocumentFragment();next.appendChild(dom(h(Top,null)));reconcile(root,next)}
function mount(component: CanvasComponent){Top=component;render()}
let stateTimer: ReturnType<typeof setTimeout> | undefined;
let stateRevision = 0;
const saveStatus = document.createElement('button');
saveStatus.className = 'oc-save-status oi-btn'; saveStatus.textContent = 'Saved'; saveStatus.setAttribute('aria-live', 'polite');
document.body.appendChild(saveStatus);
function flushState(): void { clearTimeout(stateTimer); stateTimer=undefined; vscode.postMessage({type:'stateWrite',state:persisted,revision:stateRevision}); }
function queueState(): void { stateRevision++;vscode.postMessage({type:'stateDraft',state:persisted,revision:stateRevision});saveStatus.textContent='Saving…';clearTimeout(stateTimer);stateTimer=setTimeout(flushState,200); }
saveStatus.addEventListener('click',flushState);
window.addEventListener('pagehide',()=>{if(stateTimer!==undefined)flushState()});
window.addEventListener('message',event=>{const message=event.data;if(message?.type==='stateSaved'&&message.revision===stateRevision){saveStatus.textContent=message.error?'Save failed — retry':'Saved';saveStatus.title=message.error||''}});
const states: Record<string, unknown> = {};function useCanvasState(key: string, def: unknown): [unknown, (action: unknown) => void] {if(!(key in states))states[key]=(key in persisted?persisted[key]:def);const value=states[key];function set(action: unknown){states[key]=typeof action==='function'?action(states[key]):action;persisted[key]=states[key];queueState();render()}return [value,set]}
...
const theme={kind:document.body.classList.contains('vscode-light')?'light':'dark',text:{primary:'var(--oi-text)',secondary:'var(--oi-secondary)',tertiary:'var(--vscode-disabledForeground)',quaternary:'var(--vscode-disabledForeground)',link:'var(--vscode-textLink-foreground)',onAccent:'var(--vscode-button-foreground)'},bg:{editor:'var(--oi-canvas)',chrome:'var(--oi-canvas)',elevated:'var(--oi-group)'},fill:{primary:'var(--vscode-list-activeSelectionBackground)',secondary:'var(--oi-hover)',tertiary:'var(--oi-group)',quaternary:'var(--vscode-input-background)'},stroke:{primary:'var(--oi-border)',secondary:'var(--oi-border-overlay)',tertiary:'var(--oi-border)'},accent:{primary:'var(--vscode-textLink-foreground)',control:'var(--vscode-button-background)'},diff:{added:'var(--openide-green)',removed:'var(--openide-red)'},palette:{}};(theme as unknown as Record<string, unknown>).tokens=theme;theme.palette=theme.palette;function useHostTheme(){return theme}
/* Forwards the element's IDENTITY props to the real tag: className (merged with
   whatever the component itself sets), id, title and the data- and aria- attributes. Without
   this a canvas could not mark its elements and Design Mode selectors came out positional
   (div > section:nth-of-type(2)), which tell the agent little about which component to touch. */
...
function Card(p: ICanvasProps){return h('section',Object.assign({style:p.style},ident(p,'oi-card')),p.children)}
...
function Button(p: ICanvasProps){return h('button',Object.assign({type:'button',disabled:p.disabled,onClick:p.onClick,style:p.style},ident(p,'oi-btn'+(p.variant==='primary'?' primary':p.variant==='ghost'?' ghost':''))),p.children)}
...
function Pill(p: ICanvasProps){return h('span',Object.assign({style:p.style},ident(p,'oi-pill')),p.children)}
...
function Callout(p: ICanvasProps){return h('div',Object.assign({style:p.style},ident(p,'oi-callout')),p.title&&h(H3,null,p.title),p.children)}
const Code=box('pre',{fontFamily:'var(--vscode-editor-font-family)',whiteSpace:'pre-wrap',padding:'10px',background:'var(--vscode-textCodeBlock-background)',borderRadius:'var(--oi-radius-control)',overflow:'auto'});
function Table(p: ICanvasProps){if(!p.rows||!p.rows.length)return null;return h('div',{style:mergeStyle({overflow:'auto',border:p.framed===false?'none':'1px solid '+theme.stroke.primary,borderRadius:p.framed===false?0:'var(--oi-radius-group)'},p.style)},h('table',{className:'oc-table'},h('thead',null,h('tr',null,p.headers.map(function(x: string,i: number){return h('th',{style:{textAlign:(p.columnAlign||[])[i]||'left'}},x)}))),h('tbody',null,p.rows.map(function(row: string[],ri: number){return h('tr',{style:p.striped&&ri%2?{background:theme.fill.tertiary}:undefined},p.headers.map(function(_: string,i: number){return h('td',{style:{textAlign:(p.columnAlign||[])[i]||'left'}},row[i])}))}))))}
/* Chart engine: axes with a "round" scale, grid, legend and native tooltips. The previous
   one drew only the bars/line over a fixed viewBox, with no axes and no reference for
   magnitude, so a chart could not be read without guessing the values. */
...
function chart(kind: string, p: ICanvasProps){
	...
	if(kind==='pie'){
		...
		/* The legend goes ON THE RIGHT with its value and percentage: previously the labels
		   piled onto the drawing at fixed positions and overlapped each other. */
...
function TextInput(p: ICanvasProps){return h('input',Object.assign({value:p.value||'',placeholder:p.placeholder,disabled:p.disabled,type:p.type||'text',onInput:function(e: Event){p.onChange&&p.onChange((e.target as HTMLInputElement).value)},style:p.style},ident(p,'oi-field')))}
function Select(p: ICanvasProps){return h('select',{className:'oi-field',value:p.value,disabled:p.disabled,onChange:function(e: Event){p.onChange&&p.onChange((e.target as HTMLInputElement).value)},style:p.style},p.placeholder&&h('option',{value:''},p.placeholder),(p.options||[]).map(function(o: { value: string; label: string; disabled?: boolean }){return h('option',{value:o.value,disabled:o.disabled},o.label)}))}function Checkbox(p: ICanvasProps){return h('label',{style:mergeStyle({display:'inline-flex',gap:'7px',alignItems:'center'},p.style)},h('input',{type:'checkbox',checked:p.checked,disabled:p.disabled,onChange:function(e: Event){p.onChange&&p.onChange((e.target as HTMLInputElement).checked)}}),p.label)}function Toggle(p: ICanvasProps){return h(Checkbox,p)}
function CollapsibleSection(p: ICanvasProps){const pair=useCanvasState('__collapse_'+(p.title||''),p.defaultExpanded!==false);const expanded=pair[0] as boolean;return h('section',{style:p.style},h('button',{type:'button',className:'oi-btn ghost',onClick:function(){pair[1](!expanded)},'aria-expanded':expanded},(pair[0]?'▾ ':'▸ '),p.title),expanded&&p.children as CanvasVNode)}
...
function TodoList(p: ICanvasProps){if(!p.todos||!p.todos.length)return null;return h(StackGap,{gap:4,style:p.style},p.todos.map(function(t: { status?: string; content?: string }){return h('button',{type:'button',className:'oi-btn ghost',onClick:function(){p.onTodoClick&&p.onTodoClick(t)},style:{justifyContent:'flex-start',textAlign:'left'}},t.status==='completed'?'✓':'○',t.content)}))}function TodoListCard(p: ICanvasProps){return h(Card,{style:p.style},h(CardHeader,null,(p.todos||[]).filter(function(t: { status?: string }){return t.status==='completed'}).length+' of '+(p.todos||[]).length+' Done'),h(CardBody,null,h(TodoList,p)))}
...
/* Button that runs a prompt in the chat. With send=false it leaves the text in the composer for
   the user to review before sending it; by default it sends on its own. */
function PromptButton(p: ICanvasProps){p=p||{};return h('button',Object.assign({type:'button',disabled:p.disabled,title:p.prompt||'',onClick:function(){const prompt=String(p.prompt||'').trim().slice(0,4000);if(!prompt)return;vscode.postMessage({type:'action',action:{type:'runPrompt',prompt:prompt,send:p.send!==false}})},style:p.style},ident(p,'oc-prompt-btn oi-btn'+(p.variant==='primary'?' primary':''))),p.children||p.label||'Preguntar al agente')}
function Choice(p: ICanvasProps){return h('button',Object.assign({type:'button',onClick:function(){p.onSelect&&p.onSelect(p.id)},'aria-pressed':p.selected?'true':'false',style:p.style},ident(p,'oc-choice oi-choice'+(p.selected?' selected':'')),{id:undefined}),h('span',{className:'oc-choice-mark','aria-hidden':'true'}),h('span',{className:'oc-choice-copy'},h('span',{className:'oc-choice-title'},p.title||p.children),p.description&&h('span',{className:'oc-choice-description'},p.description)))}
/* Design Mode: select an element of the canvas and send it to the agent, just like the
   browser's Pick & Polish. It reuses that contract ({selector, html, styles, rect}) so the
   chat attaches it through the same path and there are not two mechanisms to maintain. */
...
function cssPath(el: Element | null){
	/* Stable and SHORT selector: the id when there is one, otherwise the tag+class+:nth-of-type chain
	   up to the root container. The agent uses it to locate the component in the source. */
	const parts=[];
	while(el&&el.nodeType===1&&el!==root&&parts.length<6){
		...
		if(parent){
			/* The :nth-of-type is added ONLY when tag+class are not enough to tell it apart from its
			   siblings. Always adding it dirtied selectors that were already unique
			   (div.header:nth-of-type(1)) and took away the agent's hint about which component it is.
			   The index is still the one by TYPE, which is what :nth-of-type counts. */
...
designToggle.type='button';designToggle.className='oc-pick-toggle oi-btn';
...
fullBtn.type='button';fullBtn.className='oc-full-toggle oi-btn';
