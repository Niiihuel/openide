...
export class OpenideToolRegistry extends Disposable {
	...
	private updateTodosTool(): IAgentTool {
		return {
			...
			def: {
				name: 'update_todos',
				description: 'Keeps the task list visible to the user. Use it for multi-step work: ALWAYS send the COMPLETE list on every call; exactly ONE task "in-progress"; mark "completed" as soon as you finish each one.',
				parameters: {
					type: 'object',
					properties: {
						todos: {
	...
	private askUserTool(): IAgentTool {
		return {
			...
			def: {
				name: 'ask_user',
				description: 'Ask the user one or more questions when the request is ambiguous or important information is missing, BEFORE guessing. Group related questions into a single call (up to 5); offer options when it makes sense. Set allow_multiple: true on a question when several options can apply at once (the UI lets the user pick several, and the answer arrives as the chosen labels separated by commas); leave it out when the options are mutually exclusive.',
				parameters: {
					type: 'object',
					properties: {
						questions: {
							type: 'array',
							description: 'Questions for the user (1 to 5)',
							items: {
								type: 'object',
								properties: {
									question: { type: 'string', description: 'The question' },
									options: { type: 'array', items: { type: 'string' }, description: 'Suggested options (optional)' },
									allow_multiple: { type: 'boolean', description: 'Several options may be chosen at once (default false: mutually exclusive)' },
								},
								required: ['question'],
							},
						},
						question: { type: 'string', description: 'Short form: a single question (equivalent to questions with one element)' },
						options: { type: 'array', items: { type: 'string' }, description: 'Options for the short form' },
						allow_multiple: { type: 'boolean', description: 'Short form: several options may be chosen at once' },
						allow_free_text: { type: 'boolean', description: 'Allow a free-text answer (default true)' },
					},
				},
			},
	...
	private terminalSendTool(): IAgentTool {
		return {
			...
			invoke: async () => 'Error: no interactive terminal is active.', // interceptada en openideAgentService
		};
	}
	...
	private async collectDiagnostics(uri: URI, waitMs = 1200): Promise<string> {
		try {
			...
			return ''; // no model (binary, missing): no diagnostics
		}
	}
	...
	private async diagnosticsSuffix(uri: URI): Promise<string> {
		...
		return diag ? `\n\nFile diagnostics after the edit (fix any you introduced):\n${diag}` : '';
	}
		...
		return {
			...
			invoke: async (args, _token, context) => {
				...
				if (hasStart || hasEnd) {
					...
					if (end < start) { return `Error: end_line (${end}) cannot be lower than start_line (${start}).`; }
	...
	private listFilesTool(): IAgentTool {
		return {
			...
			def: {
				...
				parameters: { type: 'object', properties: { path: { type: 'string', description: 'Directory path (empty or "." = workspace root)' } } },
			},
			invoke: async (args, _token, context) => {
				const uri = this.resolvePath(String(args.path ?? '.') || '.', context?.workspaceRoot);
	...
	private fuzzyReplace(current: string, oldStr: string, newStr: string): { updated?: string; error?: string } {
		...
		if (hits.length > 1) {
			return { error: `Error: old_string (approximate matching) appears ${hits.length} times; add context to make it unique.` };
		}
		...
		return {
			...
			invoke: async (args, _token, context) => {
				...
				if (count === 1) {
					...
					if (fz.updated === undefined) { return 'Error: old_string was not found in the file, not even with whitespace-tolerant matching. Re-read the file in case it changed.'; }
					updated = fz.updated;
	...
	private runCommandTool(): IAgentTool {
		return {
			...
			def: {
				...
				parameters: {
					...
					properties: {
						...
						timeoutSeconds: { type: 'number', description: 'Wait cap in seconds (default 120, max 600) — raise it ONLY for slow builds or installs. On expiry the process is TERMINATED.' },
					},
					required: ['command'],
				},
			},
			...
			invoke: async (args, token, context) => {
				...
				if (!finished) {
					return `(timeout: the command did not finish in ${timeoutSec}s and was TERMINATED. If it is a server or watcher use background:true; if it is a slow build or install, retry with a higher timeoutSeconds.)`;
				}
				if (finished.awaitingInput) {
					const tail = finished.output.length > 2000 ? finished.output.slice(-2000) : finished.output;
					return `awaiting-input: the command is waiting for an interactive answer (y/N, password, menu). Last output:\n${tail || '(no visible output)'}\n\nUse terminal_send to reply.`;
				}
	...
	async sendToAgentTerminalInteractive(text: string, token: CancellationToken, timeoutMs = 30000, shell: string = SHARED_SHELL): Promise<ShellCaptureResult | undefined> {
		...
		if (/[\r\n\u2028\u2029\0]/.test(text)) {
			return { output: 'Error: terminal_send accepts a single line (no line breaks).', exitCode: undefined };
		}
		if (text.length > 500) {
			return { output: 'Error: terminal_send accepts at most 500 characters (it does not truncate silently).', exitCode: undefined };
		}
		const payload = text;
		...
		if (!cd) {
			...
			this.clearInteractiveSession(shell, term, { killPty: true });
			return { output: 'Error: shell integration unavailable; the prompt cannot be confirmed.', exitCode: undefined };
		}
		let outputBuffer = '';
	...
	private async startBackgroundCommand(command: string, persistent = false): Promise<string> {
		...
		if (persistent) {
			...
			await this.terminalService.showBackgroundTerminal(term);
			this.terminalService.setActiveInstance(term);
			return `Started in a persistent dock terminal (id=${id}). The terminal stays visible and does NOT close when the command finishes.`;
		}
		return `Started in the background (id=${id}). Continue without waiting for it; the user can open the terminal to watch the output.`;
	}
