...
export class OpenideWebResearch {
	...
	private searchTool(): IAgentTool {
		return {
			risk: 'safe',
			def: { name: 'web_search', description: 'Searches the public web for current information without opening the local browser. Returns citable results [S1], [S2] with title, URL and snippet. Use it for documentation, news, APIs and facts that require up-to-date sources.', parameters: { type: 'object', properties: { query: { type: 'string' }, max_results: { type: 'number', minimum: 1, maximum: 10 } }, required: ['query'] } },
			invoke: async (args: any, token: CancellationToken) => {
				if (!this.enabled()) { return 'Error: web exploration disabled (openide.agent.web.enabled).'; }
				if (token?.isCancellationRequested) { return 'Error: web search cancelled.'; }
				const query = String(args?.query ?? '').trim().slice(0, 2_000); if (!query) { return 'Error: empty web query.'; }
				const endpoint = String(this.configurationService.getValue('openide.agent.web.searchEndpoint') ?? '').trim();
				if (!endpoint) { return 'Error: set openide.agent.web.searchEndpoint to a JSON search endpoint.'; }
				...
				} catch (error) { return `Error: web search failed — ${error instanceof Error ? error.message : String(error)}`; }
			},
		};
	}
	...
	private fetchTool(): IAgentTool {
		return {
			risk: 'safe',
			def: { name: 'web_fetch', description: 'Downloads and extracts text from a public web URL with SSRF protection, redirects and limits. Does not open or share the local browser cookies. Returns a citable source [W1].', parameters: { type: 'object', properties: { url: { type: 'string' } }, required: ['url'] } },
			invoke: async (args: any, token: CancellationToken) => {
				if (!this.enabled()) { return 'Error: web exploration disabled (openide.agent.web.enabled).'; }
				if (token?.isCancellationRequested) { return 'Error: web fetch cancelled.'; }
				try {
				...
				} catch (error) { return `Error: web fetch failed — ${error instanceof Error ? error.message : String(error)}`; }
			},
		};
	}

	private parseSearchJson(raw: string): IWebSearchItem[] {
		let parsed: any; try { parsed = JSON.parse(raw); } catch { throw new Error('the endpoint returned invalid JSON'); }
