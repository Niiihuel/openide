...
export class OpenideDiffBlock extends Disposable {
	...
	private readonly _tokens = this._register(new MutableDisposable<IDisposable>());
	...
	constructor(
		...
		this._fold = this._register(new OpenideFold(this.domNode, hoverService, {
			...
			measure: () => this.domNode.querySelector<HTMLElement>('.openide-diff-lines')?.offsetHeight ?? 0,
		}));
	...
	setDiff(input: IOpenideDiffBlockInput | undefined): void {
		...
		this._tokens.value = toDisposable(() => tokens.dispose(true));
