...
export class OpenideGitFlow {
	...
	async writeConfig(cfg: IGitFlowConfig): Promise<string> {
		...
		if (!uri) {
			return 'Error: no hay carpeta abierta.';
		}
		...
		return 'OK: workflow guardado en .openide/workflow.json. La configuración previa .openide/git.json se sigue leyendo como compatibilidad.';
	}
	...
	private validateFiles(files: readonly string[]): string | undefined {
		if (!files.length) {
			return 'Debés indicar los archivos explícitos del commit; OpenIDE no usa git add -A.';
		}
		const invalid = files.find(path => !isWorkspaceRelativePath(path));
		if (invalid) {
			return `Path inválido o fuera del workspace: ${invalid}`;
		}
		const duplicate = files.find((path, index) => files.indexOf(path) !== index);
		if (duplicate) {
			return `El path ${duplicate} aparece más de una vez.`;
		}
		const secretPath = files.find(path => SUSPICIOUS_PATH_RE.test(path));
		return secretPath ? `Bloqueado: ${secretPath} parece contener secretos. Excluilo y gestioná la credencial fuera de git.` : undefined;
	}
	...
	async readReviewDiff(files: readonly string[], token: CancellationToken): Promise<{ ok: boolean; text: string; fingerprint?: string }> {
		...
		try {
			...
			const reviewDiff = current.diff.trim() || (untrackedContents ? `Archivos nuevos sin diff previo:\n${untrackedContents.replace(/\0/g, '\n')}` : '');
			if (!reviewDiff.trim()) {
				return { ok: false, text: 'Error: no hay diff versionado para revisar en los archivos seleccionados.' };
			}
	...
	async preflight(proposal: IGitProposal, token: CancellationToken): Promise<IGitPreflight> {
		...
		if (!message) {
			return { ok: false, message: 'Error: el mensaje de commit está vacío.' };
		}
		if (cfg.conventionalCommits && !CONVENTIONAL_COMMIT_RE.test(message)) {
			return { ok: false, message: 'Error: el mensaje debe usar Conventional Commits (por ejemplo: feat(chat): agrega revisión).'};
		}
		...
		if (!staged.ok) {
			return { ok: false, message: `Error: no se pudo inspeccionar el índice git.\n${staged.output}` };
		}
		if (staged.output) {
			return { ok: false, message: `Bloqueado: hay cambios ya staged fuera de este flujo:\n${staged.output}\n\nLimpiá o commiteá ese índice manualmente; OpenIDE no mezcla staging ajeno.` };
		}
		const identity = await this.run('git config user.name && git config user.email', token);
		if (!identity.ok || identity.output.split('\n').filter(Boolean).length < 2) {
			return { ok: false, message: 'Error: git necesita user.name y user.email antes de crear un commit.' };
		}
		const status = await this.run(`git status --porcelain=v1 -- ${this.pathsCommand(files)}`, token);
		if (!status.ok || !status.output) {
			return { ok: false, message: 'Error: ninguno de los archivos indicados tiene cambios para commitear.' };
		}
		const whitespace = await this.run(`git diff --check -- ${this.pathsCommand(files)}`, token);
		if (!whitespace.ok) {
			return { ok: false, message: `Error: git diff --check encontró problemas de whitespace:\n${whitespace.output}` };
		}
		...
		if (cfg.requireReview && this.reviewedFingerprint !== current.fingerprint) {
			return { ok: false, message: 'Bloqueado: el diff actual no tiene una revisión aprobada. Ejecutá review_changes después de la última edición y corregí los hallazgos.' };
		}
		return { ok: true, fingerprint: current.fingerprint, message: `Preflight OK: ${files.length} archivo(s), revisión vigente y sin staging ajeno.` };
	}
	...
	async describeStatus(token: CancellationToken): Promise<string> {
		...
		if (!st.ok && !st.output) {
			return 'Error: no se pudo leer el estado de git (¿es un repo? ¿shell integration activa?).';
		}
		...
		const out = [`Rama: ${branch}${ahead ? ` — ${ahead} commit(s) sin pushear` : ''}${behind ? ` — ${behind} detrás del remoto` : ''}.`, files.length ? `Cambios sin commitear: ${files.length} archivo(s), ~${changedLines} líneas (+${ins}/−${del}).` : 'Working tree limpio (sin cambios).'];
		...
		if (checkpointDue) {
			out.push(`CHECKPOINT RECOMENDADO: superó el umbral (${cfg.maxChangedLines} líneas o ${cfg.maxUnpushedCommits} commits sin pushear). Ejecutá review_changes y luego git_preflight antes de proponer git_commit con archivos explícitos.`);
		} else if (files.length) {
			out.push('Flujo recomendado: review_changes → git_preflight → git_commit. Los commits son atómicos, con archivos explícitos y aprobación del usuario.');
		}
		if (cfg.rules.length) {
			out.push('Reglas del usuario:\n' + cfg.rules.map(rule => `  - ${rule}`).join('\n'));
		}
		return out.join('\n');
	}
	...
	async execute(proposal: IGitProposal, token: CancellationToken): Promise<string> {
		...
		for (const step of steps) {
			...
			if (!res.ok) {
				return `Error en "${step}":\n${res.output}\n\nPasos completados antes del fallo: ${done.length ? done.join('; ') : 'ninguno'}.`;
			}
			done.push(step);
		}
		this.reviewedFingerprint = undefined;
		return `OK: commit ejecutado (sin push automático).\n${done.map(step => `  ✓ ${step}`).join('\n')}`;
	}
}
