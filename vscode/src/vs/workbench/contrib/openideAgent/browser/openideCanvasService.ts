...
export class OpenideCanvasService implements IOpenideCanvasService {
	...
	async read(path: string): Promise<string> {
		...
		if (!uri) { throw new Error('The path must be inside .openide/canvases and end in .canvas.tsx.'); }
	...
	async write(name: string, source: string): Promise<{ path: string; created: boolean; diagnostics: string[] }> {
		...
		file += '.canvas.tsx';
		if (!CANVAS_NAME.test(file)) { throw new Error('Invalid name: use kebab-case.'); }
		const dir = this.dir();
	...
	compile(source: string): IOpenideCanvasCompileResult {
		...
		for (const match of imports) { if (match[1] !== 'openide/canvas') { errors.push(`Import not allowed: ${match[1]}. Use only openide/canvas.`); } }
		if (/\b(?:fetch|XMLHttpRequest|WebSocket|EventSource)\s*\(/.test(source) || /\bimport\s*\(/.test(source)) { errors.push('Canvas allows neither network nor dynamic imports.'); }
		const defaults = source.match(/\bexport\s+default\b/g)?.length ?? 0;
		if (defaults !== 1) { errors.push(`The canvas must have exactly one default export (found: ${defaults}).`); }
		if (!imports.length && /\bfrom\s+['"]/.test(source)) { errors.push('Could not validate a canvas import.'); }
