...
export class OpenideMcpManager extends Disposable {
	...
	constructor(
		...
		this.configWatchScheduler = this._register(new RunOnceScheduler(() => {
			this.reload()
				.then(summary => this.logService.info(`[openide-mcp] mcp.json cambió — reload: ${summary}`))
				.catch(e => this.logService.warn(`[openide-mcp] reload por watcher falló: ${e instanceof Error ? e.message : String(e)}`));
	...
	async reload(): Promise<string> {
		...
		if (!this.enabled()) {
			return 'MCP está deshabilitado (openide.agent.mcp.enabled).';
		}
		if (!this.registry) {
			return 'El registry de tools todavía no está listo.';
		}
		this.startPromise = this.connectAll();
		const summary = await this.startPromise;
		if (!summary.servers && !summary.errors.length) {
			return 'Sin servers MCP configurados (.openide/mcp.json del proyecto o global del perfil).';
		}
		const ok = `${summary.servers} server(s) conectado(s), ${summary.tools} tool(s) registrada(s).`;
		return summary.errors.length ? `${ok} Errores: ${summary.errors.join(' · ')}` : ok;
	}
	...
	private scheduleReconnect(id: string): void {
		...
		if (attempt >= RECONNECT_BACKOFF_MS.length) {
			this.logService.warn(`[openide-mcp] ${id}: reintentos agotados — queda caído hasta "Recargar servers MCP"`);
			return;
		}
		...
		const timer = setTimeout(async () => {
			...
			try {
				...
				this.logService.info(`[openide-mcp] ${id}: reconectado (intento ${attempt + 1}, ${result.tools.length} tools)`);
			} catch (e) {
				// the 'error' state main emitted already scheduled (or parked) the next step
				this.logService.warn(`[openide-mcp] ${id}: reintento ${attempt + 1} falló — ${e instanceof Error ? e.message : String(e)}`);
			}
	...
	private async connectAll(): Promise<IConnectSummary> {
		...
		await Promise.all([...configs.entries()].map(async ([id, config]) => {
			...
			try {
				...
				this.logService.warn(`[openide-mcp] ${id}: no conectó — ${msg}`);
	...
	private registerServerTools(id: string, config: McpServerConfig, tools: McpToolInfo[]): number {
		...
		for (const tool of tools) {
			...
			if (this.registry.hasTool(name)) {
				...
				this.logService.warn(`[openide-mcp] ${id}: la tool "${name}" colisiona con una ya registrada — skipeada`);
				continue;
			}
	...
	private buildTool(serverId: string, tool: McpToolInfo, name: string, timeoutMs: number): IAgentTool {
		...
		const description = (tool.description || `Tool "${tool.name}" del server MCP "${serverId}".`).slice(0, TOOL_DESC_CAP);
		...
		return {
			...
			invoke: async (args) => {
				if (!this.enabled() || this.serverGenerations.get(serverId) !== generation) { return 'Error: la conexión MCP ya no está autorizada.'; }
