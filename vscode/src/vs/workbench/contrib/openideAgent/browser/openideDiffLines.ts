...
/**
 * The rows of an inline diff, transcribed from the webview's `.ediff` and Cursor's edit block:
 * one row per diff line, a sign gutter, added/removed tints, and the code painted with the SAME
 * tokenizer the editor uses for that file type — never a second highlighter. `mtkN` classes are
 * emitted unprefixed by the theme (tokenization.ts `generateTokensCSSForColorMap`), so the rows
 * pick up the editor palette as-is.
 *
 * This is the one painter for every surface that shows a change inline (the transcript's edit
 * card, the Agent Changes view); the shell around it — scroll, fade, chevron — is
 * `OpenideDiffBlock`. Anything that wants "a diff the way the harness draws it" goes through
 * those two and never through a private copy.
 *
 ...
 * re-painted in place once the grammar resolves, so the block never waits on a language load.
 */
export function appendOpenideDiffLines(parent: HTMLElement, path: string, lines: readonly OpenideDiffLine[], created: boolean, languageService: ILanguageService, token: CancellationToken, onDidPaint?: () => void): HTMLElement {
	...
	for (const line of lines) {
		...
		const text = line.x || ' ';
		const code = append(row, $('span.openide-diff-code'));
 ...
 */
async function paintWhenReady(languageId: string, codes: readonly { readonly node: HTMLElement; readonly text: string; readonly line: OpenideDiffLine }[], languageService: ILanguageService, token: CancellationToken): Promise<void> {
...
function paint(codes: readonly { readonly node: HTMLElement; readonly text: string; readonly line: OpenideDiffLine }[], support: IReducedTokenizationSupport, languageService: ILanguageService): void {
