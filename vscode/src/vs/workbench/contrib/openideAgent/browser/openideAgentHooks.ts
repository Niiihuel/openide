...
const TEST_PAYLOADS: Record<HookEvent, IHookPayload> = {
	...
	userPromptSubmit: { extra: { prompt: 'Test user message' } },
	sessionStart: {},
	stop: {},
	subagentStop: { extra: { title: 'Test subagent', is_error: false } },
};
...
export class OpenideAgentHooks extends Disposable {
	...
	private parseHooksFile(text: string, scope: 'project' | 'global'): IHookEntry[] {
		...
		try {
			...
			this.logService.warn(`[openide-hooks] ${label}: invalid JSON — file ignored`);
			return [];
		}
		...
		for (const [eventName, list] of Object.entries(hooks)) {
			if (!KNOWN_EVENTS.includes(eventName as HookEvent)) {
				...
				this.logService.warn(`[openide-hooks] ${label}: unknown event "${eventName}"${near ? ` (did you mean "${near}"?)` : ''} — skipped`);
				continue;
			}
			const event = eventName as HookEvent;
			if (!Array.isArray(list)) {
				this.logService.warn(`[openide-hooks] ${label}: "${event}" must be a list — skipped`);
				continue;
			}
			for (const item of list) {
				const command = typeof item?.command === 'string' ? item.command.trim() : '';
				if (!command) {
					this.logService.warn(`[openide-hooks] ${label}: entry of "${event}" without command — skipped`);
					continue;
				}
				let matcher = typeof item?.matcher === 'string' && item.matcher ? item.matcher : undefined;
				if (matcher && event !== 'preToolUse' && event !== 'postToolUse') {
					this.logService.warn(`[openide-hooks] ${label}: matcher is only honored in preToolUse/postToolUse — ignored in "${event}"`);
					matcher = undefined;
				}
				out.push({
					event,
		...
		try {
			...
			for (const entry of entries) {
				...
				if (!(await this.ensureConsent(entry))) {
					this.logService.warn(`[openide-hooks] ${event} "${entry.command}": no user consent — skipped (never blocks the turn)`);
					continue;
				}
				try {
					...
					this.logService.warn(`[openide-hooks] ${event} "${entry.command}": ${e instanceof Error ? e.message : String(e)} — ignored (fail-open)`);
				}
			}
		} catch (e) {
			this.logService.warn(`[openide-hooks] dispatch(${event}) failed: ${e instanceof Error ? e.message : String(e)} — ignored (fail-open)`);
		}
		return outcomes;
	}
	...
	private parseOutcome(entry: IHookEntry, result: HookExecResult): IHookOutcome {
		...
		if (parsed.decision === 'block') {
			blockMessage = String(parsed.reason ?? '') || t('service.hooks.blockedDefault');
		} else if (parsed.action === 'block') {
			blockMessage = String(parsed.message ?? '') || t('service.hooks.blockedDefault');
		}
	...
	private async promptConsent(entry: IHookEntry, mtime: number, drifted: boolean): Promise<boolean> {
		const items: (IQuickPickItem & { id: 'session' | 'always' | 'deny' })[] = [
			...
			{ id: 'always', label: t('service.hooks.allowAlways') },
			{ id: 'deny', label: t('service.hooks.deny') },
		];
		const picked = await this.quickInputService.pick(items, {
			title: drifted
				? t('hooks.consentDrift', entry.event)
				: t('service.hooks.consentTitle', entry.event),
			placeHolder: t('service.hooks.consentPh', entry.command),
