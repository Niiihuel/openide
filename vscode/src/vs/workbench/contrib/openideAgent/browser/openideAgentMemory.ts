...
export class OpenideAgentMemory {
	...
	async mutate(target: MemoryTarget, action: 'add' | 'replace' | 'remove', text: string, oldText: string): Promise<string> {
		const uri = this.uriFor(target);
		if (!uri) {
			return 'Error: no hay carpeta abierta (la memoria de proyecto necesita un workspace).';
		}
		...
		switch (action) {
			case 'add': {
				...
				if (current.includes(text.trim())) {
					return 'Error: esa entrada ya está en la memoria (duplicado).';
				}
			...
			case 'replace': {
				if (!oldText || !current.includes(oldText)) {
					return 'Error: old_text no se encontró en la memoria.';
				}
		...
		if (action === 'add' && updated.length > limit) {
			return `Error: la memoria "${target}" superaría su límite (${updated.length}/${limit} chars). Consolidá primero: usá replace/remove para fusionar o borrar entradas viejas menos valiosas, y recién después agregá la nueva.`;
		}
		await this.fileService.writeFile(uri, VSBuffer.fromString(updated));
		return `OK: memoria ${target === 'project' ? 'del proyecto' : 'del usuario'} actualizada (${action}).`;
	}
}
