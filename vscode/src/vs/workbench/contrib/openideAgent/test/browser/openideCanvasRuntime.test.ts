...
suite('OpenIDE canvas runtime', () => {
	...
	function runtimeScript(state: unknown = {}): string {
		...
		assert.strictEqual(scripts.length >= 2, true, 'el html debe traer el estado y el runtime en scripts separados');
		return scripts[1];
	}
	...
	test('embeds the runtime as serialized code, not as an interpolated string', () => {
		...
		assert.strictEqual(script.includes('${'), false, 'el runtime ya no puede depender de interpolaciones del host');
	});

	test('keeps regex escapes intact', () => {
		const script = runtimeScript();
		// The exact signature of the bug: if the escaping is lost, this becomes /s+/.
		assert.strictEqual(/split\(\/\\s\+\//.test(script), true, 'los \\s de las expresiones regulares deben sobrevivir');
		assert.strictEqual(script.includes('split(/s+/'), false, 'una barra perdida convierte \\s en la letra s');
	});
	...
	test('passes the initial state through the agreed global, not through the code', () => {
		...
		assert.strictEqual(runtimeScript({ abierto: true }).includes('"abierto":true'), false, 'el estado no se inyecta dentro del runtime');
	});
	...
	test('escapes closing script tags in the canvas payload', () => {
		...
		assert.strictEqual(html.includes('"</script><script>alert(1)'), false, 'el código del canvas no puede cerrar el script del host');
	});
});
