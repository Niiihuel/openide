...
suite('OpenIDE Chat Sessions Pane', () => {
	...
	test('disclosures preserve row identity, focus and lazy children while toggling', () => {
		...
		pane.setCompact(true);
		const project = host.querySelector<HTMLButtonElement>('[data-project-id="workspace"]')!;
	...
	test('worker avatars match the run profile, survive streaming, and update without rebuilding the sidebar', () => {
		...
		assert.strictEqual(icon.src, createSubagentAvatar(run).src);
		assert.ok(button.querySelector('.oi-spinner'), 'activity remains a separate indicator');
		assert.strictEqual(button.firstElementChild, icon);
		...
		assert.strictEqual(button.querySelector('img'), icon);
		assert.deepStrictEqual(lookups, ['run']);
		const rerouted = { ...run, profile: 'review' as const };
		runEvents.fire({ type: 'changed', run: rerouted });
		assert.strictEqual(host.querySelector('[data-subagent-session-id="child"]'), button);
		assert.strictEqual(button.querySelector('img')!.src, createSubagentAvatar(rerouted).src);
		records[2].status = 'completed'; pane.render();
		assert.strictEqual(host.querySelector('[data-subagent-session-id="child"] img')?.getAttribute('src'), createSubagentAvatar(rerouted).getAttribute('src'));
	...
	test('uses known project folders without misattributing native multi-root history', () => {
		...
		assert.strictEqual(sessionProjectContext(records[0], folders).id, 'workspace');
	...
	test('compact groups keep pins visible, limit recents, and expose collapsed archive history', () => {
		...
		assert.deepStrictEqual(groups().map(group => group.dataset.projectId), ['pinned', 'workspace', 'archived']);
		...
		groups().find(group => group.dataset.projectId === 'workspace')!.click();
		assert.strictEqual(Array.from(host.querySelectorAll('.openide-chat-sessions-row')).filter(row => !row.closest('[inert]')).length, 1);
		assert.strictEqual(groups().find(group => group.dataset.projectId === 'workspace')!.getAttribute('aria-expanded'), 'false');
