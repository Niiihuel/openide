...
suite('OpenIDE settings contract', () => {
	...
	test('every declared setting is read somewhere', () => {
		...
		assert.strictEqual(settings.length > 0, true, 'the schema scan found no settings');
		...
		assert.deepStrictEqual(huerfanos, [], 'settings declared that nobody reads (either implement them, or drop them)');
	});
			...
			for (const { file, text } of sources) {
				...
				while (index >= 0) {
					...
					if (fallback && fallback[1] !== setting.def) {
						conflictos.push(`${setting.key}: schema=${setting.def} code=${fallback[1]} (${path.basename(file)})`);
					}
					index = text.indexOf(setting.key, index + 1);
				}
			}
		}
		assert.deepStrictEqual(conflictos, [], 'the schema default and the code default have to match: getValue returns the schema one');
	});
});
