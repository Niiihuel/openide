...
suite('OpenIDE breadcrumb height', () => {
	...
	function declaredHeight(): number {
		...
		assert.notStrictEqual(match, null, 'BreadcrumbsControl.HEIGHT was not found');
	...
	test('what is reserved and what is painted measure the same', () => {
		const reservado = declaredHeight();
		const pintados = paintedHeights();
		assert.strictEqual(pintados.length > 0, true, 'the title control CSS has to pin the breadcrumb height');
		for (const pintado of pintados) {
			assert.strictEqual(pintado, reservado, `the CSS paints ${pintado}px and the layout reserves ${reservado}px: the difference is a dead band`);
		}
	});

	test('the plan controls fit inside the row with room to spare', () => {
		...
		assert.notStrictEqual(chip, null, 'the height of the plan chip was not found');
		const aire = declaredHeight() - Number(chip![1]);
		assert.strictEqual(aire >= 6, true, `only ${aire}px of total slack: the controls fill the row`);
	});
	...
	test('the breadcrumb does not draw a rule against the editor', () => {
		...
		for (const regla of reglas) {
			assert.strictEqual(/border-bottom:\s*[^;]*\d/.test(regla), false, 'the breadcrumb bottom border came back');
		}
	});
});
