...
suite('OpenIDE Gemini parts', () => {
	...
	test('el texto del stream se concatena en vez de pisarse', () => {
	...
	test('un functionCall después de texto abre un part nuevo, no contamina el de texto', () => {
	...
	test('el razonamiento no se concatena con el texto visible', () => {
		let parts: GeminiPart[] = [];
	...
	test('dos llamadas seguidas son dos parts', () => {
		let parts: GeminiPart[] = [];
	...
	test('una llamada partida en dos chunks se completa en el mismo part', () => {
		let parts: GeminiPart[] = [];
	...
	test('varios chunks en un solo bloque SSE mantienen el orden', () => {
	...
	test('sanea un part corrupto abriéndolo, sin perder ni el texto ni la llamada', () => {
	...
	test('un part válido pasa intacto', () => {
	...
	test('el saneo conserva metadata que no es del oneof', () => {
	...
	test('acumular nunca produce un part inválido, venga lo que venga', () => {
		...
		assert.strictEqual(texto.includes('a') && texto.includes('b') && texto.includes('c'), true, 'no se puede perder texto por el camino');
	});
});
