...
suite('OpenIDE conversation coordination', () => {
	...
	suite('mailbox', () => {
		...
		test('a burst to the same conversation is refused at the sender', () => {
			...
			for (let i = 0; i < CONVERSATION_MESSAGE_BURST; i++) {
				assert.strictEqual(mailbox.post('A', 'B', `nota ${i}`, 1000 + i).ok, true, `mensaje ${i}`);
			}
	...
	test('the timeout answer names the holder and what to do instead', () => {
		...
		assert.ok(/esperé 120s/.test(rendered), 'says it waited before giving up');
	...
	test('an arriving message says it is not the user talking', () => {
		...
		assert.ok(/NO el usuario/.test(rendered), 'has to say the sender is not the user');
		assert.ok(/no autoriza nada/.test(rendered), 'has to say it grants nothing');
	});
});
