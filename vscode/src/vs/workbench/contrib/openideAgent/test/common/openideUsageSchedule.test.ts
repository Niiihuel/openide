...
suite('OpenIDE usage — scheduling (Orca)', () => {
	...
	test('the failure backoff doubles from 30 s and tops out at the default poll', () => {
	...
	test('a manual click skips the throttle, but never a live Retry-After', () => {
		const now = 1_000_000;
	...
	test('with the popover open the cadence is the one-minute one, not the background one', () => {
		const now = 1_000_000;
	...
	test('a failing account stays in its own backoff lane', () => {
		const now = 1_000_000;
	...
	test('staleness: fresh, stale (shown dimmed) and expired (hidden); a 429 lasts a day', () => {
	...
	test('the tightest window is the one with the highest percentage, and only counts with status ok', () => {
	...
	test('Spanish formats: reset, duration and "ago"', () => {
...
suite('OpenIDE usage — new normalizers', () => {

	ensureNoDisposablesAreLeakedInTestSuite();

	test('Codex: keeps plan_type and reports unavailable when there are no windows', () => {
	...
	test('Gemini: buckets per model, deduplicated by quota and reset, in one-hour windows', () => {
	...
	test('OpenRouter: credits with a balance and a synthetic percentage window', () => {
