...
suite('OpenIDE tool pairing', () => {
	...
	test('a cancelled call ends up sealed', () => {
	...
	test('the synthetic result SAYS it was cancelled', () => {
	...
	test('a healthy history is left untouched', () => {
	...
	test('it seals only the missing ones from a turn with several calls', () => {
		const messages: IPairableMessage[] = [
	...
	test('results stay attached to THEIR turn, not to the end', () => {
	...
	test('an id repeated in another turn does not count as an answer', () => {
	...
	test('an assistant message with no toolCalls generates nothing', () => {
