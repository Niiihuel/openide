 ...
 */
suite('OpenIDE codebase graph layout — layers', () => {
	...
	function layerOf(result: ReturnType<typeof layoutLayered>, id: string): number {
		...
		assert.notStrictEqual(found, undefined, `node ${id} is missing`);
		return found!.layer;
	}

	test('a dependency chain produces one layer per link', () => {
	...
	test('a dependency ALWAYS lands in a later layer than whoever uses it', () => {
	...
	test('the layer is defined by the LONGEST path, not the shortest', () => {
	...
	test('a cycle neither hangs nor leaves nodes out', () => {
	...
	test('it is deterministic: same graph, same coordinates', () => {
	...
	test('no node escapes the viewport', () => {
		...
		for (const n of result.nodes) {
			assert.strictEqual(n.x >= 0 && n.x <= W, true, `${n.id} out of bounds in x: ${n.x}`);
			assert.strictEqual(n.y >= 0 && n.y <= H, true, `${n.id} out of bounds in y: ${n.y}`);
		}
	});

	test('with no edges everything does not pile up on one point', () => {
		...
		assert.strictEqual(xs.size, 12, 'the nodes of one layer overlap');
	});

	test('an empty graph does not break', () => {
 ...
 */
suite('OpenIDE codebase graph layout — communities', () => {
	...
	test('every node stays inside the canvas, radius included', () => {
		const { nodes, edges } = clustered();
		for (const node of layoutGraph(nodes, W, H, edges).nodes) {
			assert.ok(node.x - node.r >= 0 && node.x + node.r <= W, `${node.id} out of bounds in x: ${node.x}`);
			assert.ok(node.y - node.r >= 0 && node.y + node.r <= H, `${node.id} out of bounds in y: ${node.y}`);
		}
	});

	test('no pair of nodes overlaps', () => {
		const { nodes, edges } = clustered();
		...
		for (let i = 0; i < placed.length; i++) {
			for (let j = i + 1; j < placed.length; j++) {
				...
				assert.ok(distance >= (placed[i].r + placed[j].r) * 0.9, `${placed[i].id} and ${placed[j].id} overlap (${distance.toFixed(1)})`);
			}
		}
	});

	test('each module\'s hub sits closer to its own module than to the others', () => {
		const { nodes, edges } = clustered();
		const placed = layoutGraph(nodes, W, H, edges).nodes;
		...
		for (const community of ['ui', 'state', 'services']) {
			...
			for (const other of ['ui', 'state', 'services'].filter(c => c !== community)) {
				...
				assert.ok(Math.hypot(hub.x - own.x, hub.y - own.y) < Math.hypot(hub.x - foreign.x, hub.y - foreign.y), `${hub.id} lands closer to ${other}`);
			}
		}
	});
});
