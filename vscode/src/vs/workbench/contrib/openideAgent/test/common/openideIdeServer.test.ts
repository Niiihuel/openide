...
suite('OpenIDE IDE server — wire contract', () => {
	...
	suite('puerto estable por workspace', () => {
		test('las mismas carpetas dan siempre el mismo puerto', () => {
		...
		test('el orden de las carpetas no cambia el puerto', () => {
			assert.strictEqual(stableIdePort(['/a', '/b']), stableIdePort(['/b', '/a']));
		});

		test('proyectos distintos caen en puertos distintos', () => {
			assert.notStrictEqual(stableIdePort(['/home/u/uno']), stableIdePort(['/home/u/dos']));
		});

		test('el separador evita que dos particiones colisionen', () => {
			assert.notStrictEqual(stableIdePort(['ab', 'c']), stableIdePort(['a', 'bc']));
		});

		test('siempre cae dentro del rango de descubrimiento', () => {
