...
suite('OpenIDE — importing configuration from other editors and CLIs', () => {
	...
	test('the paths follow the platform', () => {
	...
	test('extensions.json yields unique lowercase ids', () => {
	...
	test('mcpServers from Claude Code and Gemini, mcp from opencode', () => {
	...
	test('Claude Code\'s per-project servers count too, and the global one wins by name', () => {
	...
	test('Codex config.toml: mcp_servers tables with args, inline env and an env sub-table', () => {
		const toml = [
			'model = "o3"  # unrelated',
			'[mcp_servers.fs]',
			'command = "npx"',
	...
	test('the existing mcp.json wins by name', () => {
	...
	test('repeated shortcuts are not duplicated', () => {
