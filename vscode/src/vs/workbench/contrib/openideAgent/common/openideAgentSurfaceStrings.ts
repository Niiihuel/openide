...
export const OPENIDE_AGENT_SURFACE_STRINGS = {
	...
	'agentSurface.chat.browserStalled': { es: 'El navegador no pudo completar tres acciones seguidas. Detuve el turno para evitar más clicks sobre una página que quizá sigue cargando. Revisá la vista previa o la consola y continuá cuando la página responda.', en: 'The browser could not complete three actions in a row. I stopped the turn to avoid more clicks on a page that may still be loading. Check the preview or console, then continue when the page responds.' },
