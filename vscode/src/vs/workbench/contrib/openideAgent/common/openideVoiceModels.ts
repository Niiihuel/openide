...
export function selectVoiceModels<M extends IVoiceCandidateModel>(
	...
	canTranscribe: (providerId: string, model: M) => boolean = (_providerId, model) => modelAcceptsAudio(model),
	...
	for (const group of groups) {
		...
		const models = group.models.filter(model => canTranscribe(group.id, model));
		if (!models.length) {
