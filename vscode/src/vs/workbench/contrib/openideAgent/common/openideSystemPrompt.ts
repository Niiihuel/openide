...
- browser_debug_start / browser_debug_status / browser_debug_stop / browser_debug_discard: opt-in network and console diagnostics for the visible preview. Start before the navigation or interaction you need to reproduce, use status to check capture progress, then stop to get a bounded report and a local JSON artifact; discard stops without saving. The capture records metadata, not request or response bodies or headers.
