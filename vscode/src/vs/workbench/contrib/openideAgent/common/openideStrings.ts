...
const STRINGS = {
	'memory.tool.save': { en: 'Saving memory', es: 'Guardando memoria' },
	'memory.tool.saveDone': { en: 'Saved memory', es: 'Memoria guardada' },
	'memory.tool.search': { en: 'Searching memory', es: 'Buscando en la memoria' },
	'memory.tool.searchDone': { en: 'Searched memory', es: 'Memoria consultada' },
	'memory.tool.get': { en: 'Reading memory', es: 'Leyendo memoria' },
	'memory.tool.getDone': { en: 'Read memory', es: 'Memoria leída' },
	'memory.tool.session_summary': { en: 'Saving session handoff', es: 'Guardando resumen de sesión' },
	'memory.tool.session_summaryDone': { en: 'Saved session handoff', es: 'Resumen de sesión guardado' },
	'memory.tool.forget': { en: 'Forgetting memory', es: 'Eliminando memoria' },
	'memory.tool.forgetDone': { en: 'Forgot memory', es: 'Memoria eliminada' },
	'memory.requestTooLarge': { en: 'The request still exceeds the model budget after compaction ({0}/{1} estimated tokens, including output reserve). Reduce attached context or select a larger context model.', es: 'La solicitud sigue excediendo el presupuesto del modelo tras compactar ({0}/{1} tokens estimados, incluida la reserva de salida). Reduce el contexto adjunto o elige un modelo con más contexto.' },
	'memory.provenance': { en: 'Authored memory · {0} · revision {1}', es: 'Memoria escrita · {0} · revisión {1}' },
	'memory.references': { en: 'Declared references (verify current code): {0}', es: 'Referencias declaradas (verificar código actual): {0}' },
	'memory.semanticEnabled': { en: 'Mirror project notes to a local Mem0 OSS server for optional semantic recall. The server controls its embedding provider. Off by default.', es: 'Replica notas del proyecto en un servidor local Mem0 OSS para búsqueda semántica opcional. El servidor controla su proveedor de embeddings. Desactivado por defecto.' },
	'memory.semanticEndpoint': { en: 'Loopback Mem0 OSS REST endpoint. Authentication uses OPENIDE_MEM0_API_KEY in the native process environment.', es: 'Endpoint REST local de Mem0 OSS. La autenticación usa OPENIDE_MEM0_API_KEY del entorno del proceso nativo.' },
	'memory.captureMode': { en: 'Memory capture: automatic records durable project knowledge during work; manual saves only through requested tool calls; off disables authored writes.', es: 'Captura de memoria: automática registra conocimiento duradero durante el trabajo; manual guarda mediante llamadas solicitadas; desactivada impide escribir memorias.' },
	'memory.maxNoteBytes': { en: 'Maximum bytes for note content and ordinary metadata. Idempotency receipts have a separate bounded budget.', es: 'Máximo de bytes para el contenido y los metadatos habituales de cada nota. Los recibos de idempotencia tienen un límite independiente.' },
	'memory.maxNotes': { en: 'Maximum durable notes per project. Existing decisions are never silently evicted.', es: 'Máximo de notas duraderas por proyecto. Las decisiones existentes nunca se eliminan automáticamente.' },
	'memory.captureBarrier': { en: 'Previous project memory capture is still pending.', es: 'La captura anterior de memoria del proyecto sigue pendiente.' },
	'memory.captureDeferred': { en: 'Memory could not be saved; checkpoint remains pending: {0}', es: 'No se pudo guardar la memoria; el punto de control queda pendiente: {0}' },
	'memory.saved': { en: 'Memory saved: {0}', es: 'Memoria guardada: {0}' },
	...
	'chat.empty.text': { es: 'Contame tu idea o elegí un punto de partida.', en: 'Describe your idea or choose a starting point.' },
	'chat.empty.explore': { es: 'Explorar el proyecto', en: 'Explore the project' },
	'chat.empty.explorePrompt': { es: 'Ayudame a entender este proyecto: identificá sus puntos de entrada, componentes principales y cómo se relacionan. No modifiques archivos.', en: 'Help me understand this project: identify its entry points, main components, and how they fit together. Do not modify files.' },
	'chat.empty.plan': { es: 'Planificar un cambio', en: 'Plan a change' },
	'chat.empty.planPrompt': { es: 'Ayudame a planificar un cambio. Primero preguntame qué quiero lograr y revisá el código relevante antes de proponer pasos.', en: 'Help me plan a change. First ask what I want to achieve and review the relevant code before proposing steps.' },
	'chat.empty.debug': { es: 'Resolver un problema', en: 'Troubleshoot a problem' },
	'chat.empty.debugPrompt': { es: 'Ayudame a investigar un problema. Primero preguntame qué ocurre, qué esperaba y cómo reproducirlo.', en: 'Help me investigate a problem. First ask what happens, what I expected, and how to reproduce it.' },
	...
	'chat.empty.sessions': { es: 'Retomar una conversación', en: 'Resume a conversation' },
	'chat.empty.chooseProject': { es: 'Abrir un proyecto', en: 'Open a project' },
	'chat.empty.chooseProjectHint': { es: 'Elegí una carpeta para añadir archivos y contexto al chat.', en: 'Choose a folder to add files and context to chat.' },
	'chat.empty.changeProjectHint': { es: 'Cambiar la carpeta usada por esta ventana.', en: 'Change the folder used by this window.' },
	...
	'chat.add.title': { es: 'Añadir contexto y herramientas', en: 'Add context and tools' },
	'chat.add.context': { es: 'Adjuntar', en: 'Attach' },
	'chat.add.files': { es: 'Archivos del proyecto', en: 'Project files' },
	'chat.add.images': { es: 'Imágenes', en: 'Images' },
	'chat.add.create': { es: 'Crear', en: 'Create' },
	'chat.add.goal': { es: 'Goal', en: 'Goal' },
	'chat.add.plan': { es: 'Modo plan', en: 'Plan mode' },
	'chat.add.canvas': { es: 'Canvas', en: 'Canvas' },
	'chat.add.tools': { es: 'Herramientas', en: 'Tools' },
	'chat.add.discover': { es: 'Herramientas y skills', en: 'Tools and skills' },
	'chat.add.memory': { es: 'Mapa y memoria', en: 'Project map and memory' },
	'chat.add.browser': { es: 'Navegador', en: 'Browser' },
	'chat.add.terminal': { es: 'Nueva terminal', en: 'New terminal' },
	...
	'chat.queue.editMessage': { es: 'Editar mensaje', en: 'Edit message' },
	'chat.queue.openInSideChat': { es: 'Abrir en chat lateral', en: 'Open in side chat' },
	'chat.queue.disable': { es: 'Desactivar cola', en: 'Turn off queueing' },
	'chat.queue.steer': { es: 'Redirigir', en: 'Steer' },
	...
	'chat.part.expandOutput': { es: 'Expandir la salida', en: 'Expand output' },
	'chat.part.collapseOutput': { es: 'Compactar la salida', en: 'Collapse output' },
	...
	'chat.working.response': { es: 'Esperando respuesta', en: 'Waiting for response' },
	...
	'settings.backToIde': { es: 'Volver al IDE', en: 'Back to IDE' },
	...
	'agentWindow.environment.location': { es: 'Ubicación', en: 'Location' },
	'agentWindow.environment.directory': { es: 'Directorio', en: 'Directory' },
	'agentWindow.environment.harness': { es: 'Agente', en: 'Harness' },
	'agentWindow.environment.repository': { es: 'Repositorio', en: 'Repository' },
	'agentWindow.environment.worktree': { es: 'Árbol de trabajo', en: 'Worktree' },
	'agentWindow.environment.sharedGit': { es: 'Datos Git compartidos', en: 'Shared Git directory' },
	'agentWindow.environment.noFolder': { es: 'No hay una carpeta abierta para esta conversación.', en: 'No folder is open for this conversation.' },
	'agentWindow.noProject': { es: 'Sin proyecto', en: 'No project' },
	'agentWindow.environment.openFiles': { es: 'Explorar archivos', en: 'Browse files' },
	'agentWindow.environment.openTerminal': { es: 'Abrir terminal aquí', en: 'Open terminal here' },
	'agentWindow.environment.copyPath': { es: 'Copiar ruta', en: 'Copy path' },
	'sessions.kind.title': { es: 'Nombre de la conversación (opcional)', en: 'Conversation name (optional)' },
	...
	'sessions.group.pinned': { es: 'Fijadas', en: 'Pinned' },
	'sessions.showMore': { es: 'Mostrar más', en: 'Show more' },
	'sessions.showLess': { es: 'Mostrar menos', en: 'Show less' },
	'sessions.action.pin': { es: 'Fijar conversación', en: 'Pin conversation' },
	'sessions.action.unpin': { es: 'Desfijar conversación', en: 'Unpin conversation' },
	'sessions.action.rename': { es: 'Renombrar conversación', en: 'Rename conversation' },
	'sessions.group.workspace': { es: 'Espacio de trabajo', en: 'Workspace' },
	'sessions.group.noProject': { es: 'Sin proyecto', en: 'No project' },
	...
	'cli.tools.preparing': { en: 'Preparing OpenIDE tools…', es: 'Preparando herramientas de OpenIDE…' },
	'cli.tools.configured': { en: 'OpenIDE tools configured; CLI connection not yet verified', es: 'Herramientas de OpenIDE configuradas; conexión del CLI sin verificar' },
	'cli.tools.manual': { en: 'OpenIDE tools need an explicit CLI connection', es: 'Las herramientas de OpenIDE requieren conectar el CLI explícitamente' },
	'cli.tools.unavailable': { en: 'Automatic OpenIDE integration is not available for this CLI', es: 'La integración automática de OpenIDE no está disponible para este CLI' },
	'cli.tools.failed': { en: 'OpenIDE tool setup failed; reopen the session to retry', es: 'Falló la preparación de herramientas; reabrí la sesión para reintentar' },
	'cli.tools.windowListed': { en: 'A client listed {0} tools in this window', es: 'Un cliente consultó {0} herramientas en esta ventana' },
	'cli.tools.windowInitialized': { en: 'An MCP client initialized in this window', es: 'Un cliente MCP se inicializó en esta ventana' },
	'cli.tools.help': { en: 'OpenIDE tools and connection details', es: 'Herramientas de OpenIDE y detalles de conexión' },
	'cli.tools.guidance': { en: 'Ask to review the current page, find module dependencies, recall or save a decision, or prepare an editable plan. The AI can discover these capabilities through OpenIDE MCP. Your CLI retains its permission controls. Window activity does not identify this session or prove that its model used a tool.', es: 'Pedí revisar la página actual, buscar dependencias, recordar o guardar una decisión, o preparar un plan editable. La IA puede descubrir estas capacidades mediante OpenIDE MCP. Tu CLI conserva sus controles de permisos. La actividad de la ventana no identifica esta sesión ni demuestra que su modelo utilizó una herramienta.' },
	...
	'cliChanges.noBaseline': { es: 'No hay una captura exacta anterior a la ejecución. La comparación es orientativa y la restauración está deshabilitada.', en: 'An exact snapshot from before execution is unavailable. The comparison is for review and restoration is disabled.' },
	'cliChanges.breadcrumb.noBaseline': { es: 'Sin punto de retorno', en: 'No restore point' },
	'cliChanges.breadcrumb.changed': { es: 'Cambio observado durante la sesión', en: 'Change observed during the session' },
	'cliChanges.breadcrumb.undo': { es: 'Restaurar captura', en: 'Restore Snapshot' },
	'cliChanges.breadcrumb.undoTitle': { es: 'Revisa y restaura la captura de este archivo. Puede incluir cambios de otros autores realizados durante la sesión.', en: 'Review and restore this file snapshot. It may include other writers’ changes made during the session.' },
	...
	'cliChanges.breadcrumb.restoreConfirm': { es: '¿Restaurar la captura de {0}?', en: 'Restore the snapshot of {0}?' },
	'cliChanges.breadcrumb.restoreDetail': { es: 'La comparación muestra cambios observados, que pueden incluir trabajo tuyo o de otras sesiones. Restaurar reemplaza el contenido con la captura previa; no distingue quién escribió cada cambio.', en: 'The comparison shows observed changes, which can include your work or other sessions. Restoration replaces the content with the earlier snapshot; it cannot distinguish who wrote each change.' },
	'cliChanges.breadcrumb.restoreResult': { es: 'No se restauró {0} ({1}): {2}', en: '{0} was not restored ({1}): {2}' },
	...
	'native.unavailable': { es: 'Esta función de OpenIDE requiere la aplicación de escritorio.', en: 'This OpenIDE capability requires the desktop application.' },
	'ide.register.pick': { es: 'Registrar o reconectar esta ventana. Se crea una entrada independiente y se conservan las anteriores.', en: 'Register or reconnect this window. Creates a separate entry and preserves previous entries.' },
	...
	'ide.register.done': { es: 'Se agregó "{1}" a {0}. Reiniciá el CLI para cargarla. La conexión caduca al cerrar o recargar esta ventana; usá Registrar para reconectar. Las entradas anteriores, incluida "openide", se conservan: revisalas y retiralas manualmente en el CLI cuando ya no las necesites.', en: 'Added "{1}" to {0}. Restart the CLI to load it. This connection expires when this window closes or reloads; use Register to reconnect. Previous entries, including "openide", are preserved: review and remove them manually in the CLI when no longer needed.' },
	'ide.register.already': { es: '"{1}" ya se registró en {0} para esta ventana. Reiniciá el CLI si todavía no la cargó. Caduca al cerrar o recargar esta ventana.', en: '"{1}" is already registered in {0} for this window. Restart the CLI if it has not loaded it yet. It expires when this window closes or reloads.' },
	'ide.register.expired': { es: 'La conexión "{0}" pertenece a una sesión de ventana que terminó. Reconectar crea una entrada independiente para esta ventana. Las entradas anteriores se conservan para que las revises en el CLI.', en: 'The connection "{0}" belongs to a window session that ended. Reconnect creates a separate entry for this window. Previous entries are preserved for your review in the CLI.' },
	'ide.register.reconnect': { es: 'Reconectar esta ventana', en: 'Reconnect This Window' },
	'ide.register.uniqueNameFailed': { es: 'No se pudo generar un nombre de registro único. Volvé a intentar; las entradas anteriores se conservaron.', en: 'Could not allocate a unique registration name. Try again; previous entries were preserved.' },
	...
	'chat.header.restore': { es: 'Restaurar tamaño', en: 'Restore Size' },
