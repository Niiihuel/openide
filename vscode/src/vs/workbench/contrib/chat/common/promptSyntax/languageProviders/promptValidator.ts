			...
			if (!validAttributeNames.includes(attribute.key)) {
				...
				switch (promptType) {
					...
					case PromptsType.instructions:
						...
						break;
					case PromptsType.skill:
						report(toMarker(localize('promptValidator.unknownAttribute.skill', "Attribute '{0}' is not supported by OpenIDE skills. Supported: {1}.", attribute.key, supportedNames.value), attribute.range, MarkerSeverity.Hint, [MarkerTag.Unnecessary]));
						break;
				}
			}
		}
