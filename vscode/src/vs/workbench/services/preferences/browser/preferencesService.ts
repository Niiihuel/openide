...
import { PreferencesEditorInput } from '../common/preferencesEditorInput.js';
import { SettingsEditorInput } from '../../../contrib/openideSettings/browser/openideSettingsInput.js';
...
export class PreferencesService extends Disposable implements IPreferencesService {
	...
	private _cachedSettingsEditorInput: SettingsEditorInput | undefined = undefined;
	...
	private createOrGetCachedSettingsEditorInput(): SettingsEditorInput {
		if (!this._cachedSettingsEditorInput || this._cachedSettingsEditorInput.isDisposed()) {
			// Recreate the product-owned input if Settings was never opened or was closed.
			this._cachedSettingsEditorInput = new SettingsEditorInput(this);
		}
		return this._cachedSettingsEditorInput;
	}
	...
	private async openSettings2(options: IOpenSettingsOptions): Promise<IEditorPane | undefined> {
		const input = this.createOrGetCachedSettingsEditorInput();
		options = {
			...options,
			focusSearch: true
		};
	...
	public override dispose(): void {
		if (this._cachedSettingsEditorInput && !this._cachedSettingsEditorInput.isDisposed()) {
			this._cachedSettingsEditorInput.dispose();
		}
