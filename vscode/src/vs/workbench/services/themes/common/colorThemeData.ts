...
export class ColorThemeData implements IWorkbenchColorTheme {
	...
	/**
	 * The colour a theme or `workbench.colorCustomizations` EXPLICITLY set for `colorId`, as opposed
	 * to whatever it resolves to by default. Modern UI's tab back-compat needs the distinction: a
	 * theme that customised the legacy `tab.*` colours but knows nothing about `modernTab.*` should
	 * still tint the modern tabs, and only an explicit customisation may do that.
	 */
