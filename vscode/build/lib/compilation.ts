...
const codiconSource = path.join(root, 'resources', 'openide-icons', 'openide-codicon.ttf');
const codiconFilledSource = path.join(root, 'resources', 'openide-icons', 'openide-codicon-filled.ttf');
const codiconDest = path.join(root, 'src', 'vs', 'base', 'browser', 'ui', 'codicons', 'codicon', 'codicon.ttf');
const codiconFilledDest = path.join(root, 'src', 'vs', 'base', 'browser', 'ui', 'codicons', 'codicon', 'codicon-filled.ttf');

function copyCodiconsImpl() {
	try {
		if (!fs.existsSync(codiconSource) || !fs.existsSync(codiconFilledSource)) {
			throw new Error('Generated OpenIDE regular and filled fonts are required.');
		}
		const validation = spawnSync(process.execPath, [path.join(root, 'resources', 'openide-icons', 'validate_openide_codicons.mjs')], {
			cwd: root,
			stdio: 'inherit',
		});
		if (validation.status !== 0) {
			throw new Error(`OpenIDE icon validation failed with exit code ${validation.status ?? 'unknown'}.`);
		}
		fs.mkdirSync(path.dirname(codiconDest), { recursive: true });
		fs.copyFileSync(codiconSource, codiconDest);
		fs.copyFileSync(codiconFilledSource, codiconFilledDest);
	} catch (e) {
...
export const watchCodiconsTask = task.define('watch-codicons', () => {
	copyCodiconsImpl();
	return watch(['node_modules/@vscode/codicons/dist/**', 'resources/openide-icons/*.ttf'], { readDelay: 200 })
