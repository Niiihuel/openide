...
async function fetchUrl(options: IFetchOptions): Promise<void> {
	const version = process.env['VSCODE_SYSROOT_VERSION'] ?? '20260212-405735';
	const repository = process.env['VSCODE_SYSROOT_REPOSITORY'] ?? 'Microsoft/vscode-linux-build-agent';
	const releaseUrl = `https://api.github.com/repos/${repository}/releases/tags/v${version}`;
	...
	if (!asset) {
		throw new Error(`Could not find asset in release of ${repository} @ ${version}`);
	}
