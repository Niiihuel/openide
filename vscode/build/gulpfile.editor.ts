...
const extractEditorSrcTask = task.define('extract-editor-src', () => {
	// Keep Monaco on the same OpenIDE icon family as the workbench. The generated fonts retain
	// Codicon codepoints, so standalone integrations remain API-compatible.
	const openideIconRoot = path.join(root, 'resources', 'openide-icons');
	const codiconSource = path.join(openideIconRoot, 'openide-codicon.ttf');
	const codiconFilledSource = path.join(openideIconRoot, 'openide-codicon-filled.ttf');
