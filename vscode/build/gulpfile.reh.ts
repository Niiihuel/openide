...
const nodejsArtifactFeed = 'nodejsArtifactFeed' in product ? product.nodejsArtifactFeed as string : undefined;
...
function nodejs(platform: string, arch: string): NodeJS.ReadWriteStream | undefined {
	...
	switch (platform) {
		...
		case 'darwin':
		case 'linux': {
			const downloaded = (process.env.VSCODE_NODEJS_SITE && process.env.VSCODE_NODEJS_URLROOT
...
function fetchNodejs(assetName: string, checksumSha256: string | undefined): NodeJS.ReadWriteStream {
	...
	if (!nodejsArtifactFeed) { throw new Error('No internal Node.js feed configured'); }
